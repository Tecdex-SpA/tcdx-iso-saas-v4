...
OUT_ROOT="${TCDX_OUT_DIR:-./qa-results/company-profile-impact}"
...
echo "[company-profile-impact] Login against ${BASE_URL}"
...
{
  "profile_json": {
    "company_name": "Rieltec QA Impact",
...
echo "[company-profile-impact] Saving profile"
...
echo "[company-profile-impact] Starting async AI analysis"
...
while [ "$ELAPSED" -lt "$MAX_WAIT_SECONDS" ]; do
  ...
  echo "[company-profile-impact] status=${STATUS} elapsed=${ELAPSED}s"
...
echo "[company-profile-impact] Loading impact"
IMPACT_JSON="${OUT_DIR}/impact.json"
curl -sk -H "Authorization: Bearer ${TOKEN}" \
  "${BASE_URL}/api/company-profile/impact" \
  -o "$IMPACT_JSON"
...
for expr in \
  "data.profile" \
  "data.impact_profile.suggested_kpis" \
  "data.impact_profile.suggested_controls" \
  "data.impact_profile.health_scoring_adjustments" \
  "data.impact_profile.control_domain_weights" \
  "data.impact_profile.improvement_roadmap"; do
  if [ -z "$(json_get "$IMPACT_JSON" "$expr")" ]; then
    echo "ERROR: missing impact field ${expr}. See ${IMPACT_JSON}" >&2
    exit 1
  fi
done

echo "[company-profile-impact] Exporting context document"
...
echo "[company-profile-impact] OK"
cat "$SUMMARY"
