...
async function main() {
  ...
  try {
    ...
    for (const tenant of TENANTS) {
      ...
      if (row?.count !== 50) throw new Error(`Tenant isolation failed for ${tenant.name}: expected 50 snapshots, got ${row?.count || 0}`);
    }
    ...
    process.stdout.write(JSON.stringify({ status: 'PHASE5_5_50_FORMULA_SNAPSHOT_POSTGRES_OK', tenants: TENANTS.map((item) => item.name), formulas_per_tenant: 50, snapshots: persisted, snapshot_type: SOURCE_DATASET_SNAPSHOT_TYPE, cross_tenant_rows: 0 }) + '\n');
  } finally {
    await client.end();
  }
}
