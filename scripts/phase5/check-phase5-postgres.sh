...
phase5_checksum="$(node "$PHASE5_RUNNER" --checksum | sed -E 's/^.*checksum=//')"
...
printf '{"status":"VERIFIED_PHASE5_POSTGRES","tables":%s,"global_metrics":%s,"capabilities":%s,"limits":%s,"tenant_isolation":"verified","failed_retry":"verified","applied_checksum_mismatch":"rejected"}\n' \
  "$tables_count" "$metrics_count" "$capabilities_count" "$limits_count"
