...
function arrays(value, output = []) {
  if (Array.isArray(value)) {
    ...
    for (const child of Object.values(value)) arrays(child, output);
  }
  return output;
}
...
function resolveEndpoint(endpoint) {
  return endpoint.replace(/:tenant/g, tenantId).replace(/:metric1/g, demoUuid('metric-1'));
}
...
function validatePayload(item, payload) {
  ...
  const positiveNumbers = objects(data)
    .flatMap((record) => Object.values(record))
    .filter((value) => typeof value === 'number' && value > 0);

  if (records.length < item.minimum && positiveNumbers.length === 0) {
    fail(`${item.endpoint} has no observable data (minimum=${item.minimum})`);
  }

  for (const object of objects(data)) {
    for (const [key, value] of Object.entries(object)) {
      if ((key === 'tenant_id' || key === 'tenantId') && value && value !== tenantId) {
        fail(`${item.endpoint} leaked tenant ${value}`);
      }
      if (/percent|percentage|_pct$/i.test(key) && typeof value === 'number' && (value < 0 || value > 100)) {
        fail(`${item.endpoint} invalid percentage ${key}=${value}`);
      }
      if (/(_at|_date|date)$/.test(key) && value && typeof value === 'string' && Number.isNaN(Date.parse(value))) {
        fail(`${item.endpoint} invalid date ${key}`);
      }
    }
  }
  ...
  if (
    records.length &&
    records.some((row) => row && typeof row === 'object') &&
    !records.some((row) => row.id || row.run_id || row.audit_id || row.tenant_control_id || row.code)
  ) {
  ...
  if (item.diversityKey && records.length >= 4) {
    const values = new Set(
      records.map((row) => row?.[item.diversityKey]).filter((value) => value !== null && value !== undefined),
    );
  ...
  if (item.seriesMinimum) {
    ...
    if (candidate.length < item.seriesMinimum) {
      fail(`${item.endpoint} has fewer than ${item.seriesMinimum} series points`);
    }
  }
...
async function main() {
  ...
  if (!password) fail('DEMO_ADMIN_PASSWORD is required');
  ...
  for (const email of ['admin.demo@tcdx.demo', 'auditor.demo@tcdx.demo']) {
    ...
    for (const item of routes) {
      ...
      evidence.push({
        route: item.route,
        user: email,
        endpoint: resolveEndpoint(item.endpoint),
  ...
  const output = path.resolve(
    process.env.DEMO_API_EVIDENCE_FILE || path.join(root, 'artifacts/demo/demo-visual-api-evidence.json'),
  );
  fs.mkdirSync(path.dirname(output), { recursive: true });
  fs.writeFileSync(output, `${JSON.stringify({ tenantId, generatedAt: new Date().toISOString(), evidence }, null, 2)}\n`);
  process.stdout.write(
    `${JSON.stringify({
      status: 'VERIFIED_DEMO_VISUAL_API',
      users: 2,
