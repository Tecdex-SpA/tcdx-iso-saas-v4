...
OUT_ROOT="${TCDX_OUT_DIR:-./qa-results/company-profile-impact-full}"
...
echo "[company-profile-impact-full] Login against ${BASE_URL}"
...
{
  "profile_json": {
    "company_name": "Rieltec QA Full Impact",
    "industry": "Servicios tecnológicos",
    "subindustry": "SaaS, infraestructura y cumplimiento",
    "company_size": "pyme",
    "current_maturity_level": "inicial",
    "risk_appetite": "bajo",
    "active_standards": ["ISO9001", "ISO27001"],
    "critical_processes": ["Gestión de servicios TI", "Soporte a clientes", "Gestión de cambios", "Gestión documental"],
    ...
    "known_weaknesses": ["Evidencia incompleta", "Planes de acción vencidos", "Revisión de accesos pendiente"],
...
echo "[company-profile-impact-full] Saving profile"
...
echo "[company-profile-impact-full] Starting async AI analysis"
...
while [ "$ELAPSED" -lt "$MAX_WAIT_SECONDS" ]; do
  ...
  echo "[company-profile-impact-full] status=${STATUS} elapsed=${ELAPSED}s"
...
curl -sk -H "Authorization: Bearer ${TOKEN}" "${BASE_URL}/api/company-profile" -o "$PROFILE_JSON"
...
IMPACT_JSON="${OUT_DIR}/impact.json"
curl -sk -H "Authorization: Bearer ${TOKEN}" "${BASE_URL}/api/company-profile/impact" -o "$IMPACT_JSON"
if [ "$(json_get "$IMPACT_JSON" "ok")" != "true" ]; then
  echo "ERROR: impact endpoint failed. See ${IMPACT_JSON}" >&2
  exit 1
fi

MODULES=(dashboard health controls kpis audits action-plans reports)
for module in "${MODULES[@]}"; do
  FILE="${OUT_DIR}/modules/${module}.json"
  curl -sk -H "Authorization: Bearer ${TOKEN}" \
    "${BASE_URL}/api/company-profile/impact/module/${module}" \
    -o "$FILE"
  if [ "$(json_get "$FILE" "ok")" != "true" ]; then
    echo "ERROR: module ${module} did not return ok=true. See ${FILE}" >&2
    exit 1
  fi
  if [ "$(json_get "$FILE" "tenant_filter_enforced")" != "true" ]; then
    echo "ERROR: module ${module} missing tenant_filter_enforced=true. See ${FILE}" >&2
    exit 1
  fi
  if [ "$(json_get "$FILE" "company_profile_used")" != "true" ]; then
    echo "ERROR: module ${module} missing company_profile_used=true. See ${FILE}" >&2
    exit 1
  fi
  PRIORITIZED_COUNT="$(json_len "$FILE" "prioritized_items")"
  FOCUS_COUNT="$(json_len "$FILE" "recommended_focus")"
  if [ "$PRIORITIZED_COUNT" = "0" ] && [ "$FOCUS_COUNT" = "0" ]; then
    echo "ERROR: module ${module} has no prioritized_items or recommended_focus. See ${FILE}" >&2
    exit 1
  fi
done

echo "[company-profile-impact-full] Exporting context document"
...
if [ -z "$DOWNLOAD_URL" ]; then DOWNLOAD_URL="/api/company-profile/context-document/download"; fi
...
SUMMARY="${OUT_DIR}/summary.txt"
{
  ...
  echo "impact_json=${IMPACT_JSON}"
  echo "modules=${MODULES[*]}"
  echo "pdf=${PDF_PATH}"
  echo "pdf_size=${PDF_SIZE}"
} > "$SUMMARY"

echo "[company-profile-impact-full] OK"
cat "$SUMMARY"
