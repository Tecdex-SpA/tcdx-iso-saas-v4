...
echo "Validando Dashboard v2 paneles operativos en ${API_URL}"
...
write_guard_counts() {
  if [[ -n "${DATABASE_URL:-}" ]]; then
    psql "$DATABASE_URL" -At -c "
    SELECT 'suggestions:' || COUNT(*) FROM iso_operational_suggestions
    UNION ALL SELECT 'conversions:' || COUNT(*) FROM iso_recommended_action_conversions
    UNION ALL SELECT 'risk_items:' || COUNT(*) FROM iso_risk_matrix_items
    ORDER BY 1;
    " 2>/dev/null || true
  fi
}
...
echo "$summary_json" | jq -e '.ok == true and ((.data.operational_panels // .operational_panels) | type == "object")' >/dev/null
...
if echo "$summary_json" | jq -e '[(.data.active_standards // .active_standards // [])[].publication_status] | index("transition_prep")' >/dev/null; then
  echo "ERROR: Dashboard v2 operacional incluyo version transition_prep como operativa" >&2
  exit 1
fi
...
check_subset "Acciones Dashboard v2" "$action_codes"
check_subset "Riesgos Dashboard v2" "$risk_codes"
check_subset "KPIs Dashboard v2" "$kpi_codes"
check_subset "Alertas Dashboard v2" "$alert_codes"
...
if [[ -n "$before_write_guard" && "$before_write_guard" != "$after_write_guard" ]]; then
  echo "ERROR: una consulta read-only cambio sugerencias, conversiones o riesgos" >&2
  echo "Antes:"
...
echo "OK: Dashboard v2 paneles operativos validado correctamente."
