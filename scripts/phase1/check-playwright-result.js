...
const invalid = tests.filter(({ item }) => {
  ...
  return item.expectedStatus !== 'passed' || results.length !== 1 || results[0].status !== 'passed' || Number(results[0].retry || 0) !== 0;
});
if (tests.length !== expected || invalid.length) {
  console.error(`Playwright result invalid: expected=${expected} actual=${tests.length} invalid=${invalid.length}`);
  for (const entry of invalid) console.error(`- ${entry.title}`);
  process.exit(1);
}
process.stdout.write(`Playwright result: VERIFIED ${tests.length}/${expected} passed, retries=0\n`);
