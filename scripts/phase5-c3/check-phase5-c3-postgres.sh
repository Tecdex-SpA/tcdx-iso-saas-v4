...
MIGRATION_DATABASE_URL="$DATABASE_URL" node "$PHASE5_RUNNER" --apply >/tmp/tcdx-phase5-c3-null-state-hotfix-1.txt
MIGRATION_DATABASE_URL="$DATABASE_URL" node "$PHASE5_RUNNER" --apply >/tmp/tcdx-phase5-c3-null-state-hotfix-2.txt
...
null_state_ledger="$(run_psql -Atqc "SELECT status FROM schema_migrations WHERE migration_id='20260810_phase5_official_measurement_null_states'")"
legacy_check1_count="$(run_psql -Atqc "SELECT count(*) FROM pg_constraint WHERE conrelid='metric_measurements'::regclass AND conname='metric_measurements_check1'")"
...
[[ "$null_state_ledger" == "applied" && "$legacy_check1_count" == "0" ]] || { echo "Phase 5 null-state hotfix did not remove metric_measurements_check1" >&2; exit 1; }

run_psql -v ON_ERROR_STOP=1 -c "WITH definition AS (SELECT md.id FROM metric_definitions md JOIN metric_definition_versions v ON v.metric_definition_id=md.id WHERE v.functional_code='COMPLIANCE' AND v.status='published' LIMIT 1), policy AS (SELECT id FROM metric_trust_policies WHERE tenant_id IS NULL AND status='published' LIMIT 1), trust AS (INSERT INTO metric_trust_assessments(tenant_id,metric_definition_id,trust_policy_id,score,trust_status,dimensions,evidence_checksum,assessment_checksum,correlation_id) SELECT '70000000-0000-0000-0000-000000000701',definition.id,policy.id,88,'acceptable','{}',repeat('1',64),repeat('2',64),'pg-c3' FROM definition,policy RETURNING id,metric_definition_id), measurement AS (INSERT INTO metric_measurements(tenant_id,metric_definition_id,period_key,period_start,period_end,value_numeric,unit,source_timestamp,quality_status,freshness_status,trust_score,trust_status,validation_status,correlation_id,metadata,official_state,coverage_ratio,sample_size,population_size,sufficiency_status,source_snapshot_ids,trust_assessment_id) SELECT '70000000-0000-0000-0000-000000000701',trust.metric_definition_id,'2026-08','2026-08-01'::timestamptz,'2026-08-31'::timestamptz,0,'%','2026-08-31'::timestamptz,'valid','current',88,'acceptable','valid','pg-c3-calculated-zero','{}','calculated',1,10,10,'sufficient','{}',trust.id FROM trust RETURNING id,metric_definition_id,trust_assessment_id) INSERT INTO metric_snapshots(tenant_id,metric_definition_id,measurement_id,period_key,snapshot_payload,content_hash,trust_assessment_id,effective_at,snapshot_status,published_at) SELECT '70000000-0000-0000-0000-000000000701',measurement.metric_definition_id,measurement.id,'2026-08','{\"metric_code\":\"COMPLIANCE\",\"result\":{\"status\":\"calculated\",\"value\":0},\"unit\":\"%\",\"coverage\":1,\"trust\":{\"score\":88,\"status\":\"acceptable\"},\"freshness\":{\"status\":\"fresh\"},\"sufficiency\":{\"status\":\"sufficient\"}}',repeat('3',64),measurement.trust_assessment_id,'2026-08-31'::timestamptz,'published',now() FROM measurement;" >/dev/null
run_psql -v ON_ERROR_STOP=1 -c "WITH definition AS (SELECT md.id FROM metric_definitions md JOIN metric_definition_versions v ON v.metric_definition_id=md.id WHERE v.functional_code='COMPLIANCE' AND v.status='published' LIMIT 1), policy AS (SELECT id FROM metric_trust_policies WHERE tenant_id IS NULL AND status='published' LIMIT 1), trust AS (INSERT INTO metric_trust_assessments(tenant_id,metric_definition_id,trust_policy_id,score,trust_status,dimensions,evidence_checksum,assessment_checksum,correlation_id) SELECT '70000000-0000-0000-0000-000000000701',definition.id,policy.id,NULL,'unknown','{}',repeat('4',64),repeat('5',64),'pg-c3-null-states' FROM definition,policy RETURNING id,metric_definition_id) INSERT INTO metric_measurements(tenant_id,metric_definition_id,period_key,period_start,period_end,value_numeric,value_text,unit,source_timestamp,quality_status,freshness_status,trust_status,validation_status,correlation_id,metadata,official_state,coverage_ratio,sample_size,population_size,sufficiency_status,source_snapshot_ids,trust_assessment_id) SELECT '70000000-0000-0000-0000-000000000701',trust.metric_definition_id,state,'2026-09-01'::timestamptz,'2026-09-30'::timestamptz,NULL,NULL,'%','2026-09-30'::timestamptz,'unknown','unknown','unknown','pending','pg-c3-'||state,'{}',state,coverage,0,10,sufficiency,'{}',trust.id FROM trust CROSS JOIN (VALUES ('source_unavailable'::text,NULL::numeric,'source_unavailable'::text),('unmeasured'::text,0::numeric,'sufficient'::text),('insufficient_data'::text,1::numeric,'insufficient'::text)) states(state,coverage,sufficiency);" >/dev/null
if run_psql -v ON_ERROR_STOP=1 -c "WITH definition AS (SELECT md.id FROM metric_definitions md JOIN metric_definition_versions v ON v.metric_definition_id=md.id WHERE v.functional_code='COMPLIANCE' AND v.status='published' LIMIT 1) INSERT INTO metric_measurements(tenant_id,metric_definition_id,period_key,period_start,period_end,value_numeric,unit,quality_status,freshness_status,trust_status,validation_status,metadata,official_state,sufficiency_status) SELECT '70000000-0000-0000-0000-000000000701',id,'reject-non-calculated-numeric','2026-10-01','2026-10-31',1,'%','unknown','unknown','unknown','pending','{}','source_unavailable','source_unavailable' FROM definition" >/tmp/tcdx-phase5-c3-reject-non-calculated-numeric.txt 2>&1; then echo "Non-calculated state accepted a numeric official value" >&2; exit 1; fi
if run_psql -v ON_ERROR_STOP=1 -c "WITH definition AS (SELECT md.id FROM metric_definitions md JOIN metric_definition_versions v ON v.metric_definition_id=md.id WHERE v.functional_code='COMPLIANCE' AND v.status='published' LIMIT 1) INSERT INTO metric_measurements(tenant_id,metric_definition_id,period_key,period_start,period_end,value_numeric,value_text,unit,quality_status,freshness_status,trust_status,validation_status,metadata,official_state,sufficiency_status) SELECT '70000000-0000-0000-0000-000000000701',id,'reject-calculated-null','2026-10-01','2026-10-31',NULL,NULL,'%','unknown','unknown','unknown','pending','{}','calculated','sufficient' FROM definition" >/tmp/tcdx-phase5-c3-reject-calculated-null.txt 2>&1; then echo "Calculated official state accepted NULL value" >&2; exit 1; fi
if run_psql -v ON_ERROR_STOP=1 -c "WITH definition AS (SELECT md.id FROM metric_definitions md JOIN metric_definition_versions v ON v.metric_definition_id=md.id WHERE v.functional_code='COMPLIANCE' AND v.status='published' LIMIT 1) INSERT INTO metric_measurements(tenant_id,metric_definition_id,period_key,period_start,period_end,value_numeric,value_text,unit,quality_status,freshness_status,trust_status,validation_status,metadata) SELECT '70000000-0000-0000-0000-000000000701',id,'reject-legacy-null','2026-10-01','2026-10-31',NULL,NULL,'%','unknown','unknown','unknown','pending','{}' FROM definition" >/tmp/tcdx-phase5-c3-reject-legacy-null.txt 2>&1; then echo "Legacy measurement accepted NULL/NULL value" >&2; exit 1; fi
run_psql -v ON_ERROR_STOP=1 -c "WITH definition AS (SELECT md.id FROM metric_definitions md JOIN metric_definition_versions v ON v.metric_definition_id=md.id WHERE v.functional_code='COMPLIANCE' AND v.status='published' LIMIT 1) INSERT INTO metric_measurements(tenant_id,metric_definition_id,period_key,period_start,period_end,value_numeric,unit,quality_status,freshness_status,trust_status,validation_status,metadata,coverage_ratio) SELECT '70000000-0000-0000-0000-000000000701',id,'legacy-value','2026-10-01','2026-10-31',82.4,'%','valid','current','trusted','valid','{}',0 FROM definition;" >/dev/null
run_psql -v ON_ERROR_STOP=1 -c "WITH definition AS (SELECT md.id FROM metric_definitions md JOIN metric_definition_versions v ON v.metric_definition_id=md.id WHERE v.functional_code='COMPLIANCE' AND v.status='published' LIMIT 1) INSERT INTO metric_measurements(tenant_id,metric_definition_id,period_key,period_start,period_end,value_numeric,unit,quality_status,freshness_status,trust_status,validation_status,metadata,coverage_ratio) SELECT '70000000-0000-0000-0000-000000000701',id,'coverage-one','2026-10-01','2026-10-31',82.4,'%','valid','current','trusted','valid','{}',1 FROM definition;" >/dev/null
if run_psql -v ON_ERROR_STOP=1 -c "WITH definition AS (SELECT md.id FROM metric_definitions md JOIN metric_definition_versions v ON v.metric_definition_id=md.id WHERE v.functional_code='COMPLIANCE' AND v.status='published' LIMIT 1) INSERT INTO metric_measurements(tenant_id,metric_definition_id,period_key,period_start,period_end,value_numeric,unit,quality_status,freshness_status,trust_status,validation_status,metadata,coverage_ratio) SELECT '70000000-0000-0000-0000-000000000701',id,'reject-coverage','2026-10-01','2026-10-31',82.4,'%','valid','current','trusted','valid','{}',1.01 FROM definition" >/tmp/tcdx-phase5-c3-reject-coverage.txt 2>&1; then echo "Coverage ratio > 1 was accepted" >&2; exit 1; fi
if run_psql -v ON_ERROR_STOP=1 -c "WITH definition AS (SELECT md.id FROM metric_definitions md JOIN metric_definition_versions v ON v.metric_definition_id=md.id WHERE v.functional_code='COMPLIANCE' AND v.status='published' LIMIT 1) INSERT INTO metric_measurements(tenant_id,metric_definition_id,period_key,period_start,period_end,value_numeric,unit,quality_status,freshness_status,trust_status,validation_status,metadata) SELECT '70000000-0000-0000-0000-000000000701',id,'reject-period','2026-10-31','2026-10-01',82.4,'%','valid','current','trusted','valid','{}' FROM definition" >/tmp/tcdx-phase5-c3-reject-period.txt 2>&1; then echo "Invalid period was accepted" >&2; exit 1; fi
...
concurrent_snapshot_sql="INSERT INTO metric_snapshots(tenant_id,metric_definition_id,measurement_id,period_key,snapshot_payload,content_hash,trust_assessment_id,effective_at,snapshot_status) SELECT tenant_id,metric_definition_id,measurement_id,'2026-10',snapshot_payload,repeat('5',64),trust_assessment_id,'2026-10-31','draft' FROM metric_snapshots WHERE tenant_id='70000000-0000-0000-0000-000000000701' ORDER BY effective_at LIMIT 1 ON CONFLICT(tenant_id,metric_definition_id,period_key,content_hash) DO NOTHING;"
run_psql -v ON_ERROR_STOP=1 -c "$concurrent_snapshot_sql" >/tmp/tcdx-phase5-c3-concurrent-1.txt & concurrent_pid_1=$!
run_psql -v ON_ERROR_STOP=1 -c "$concurrent_snapshot_sql" >/tmp/tcdx-phase5-c3-concurrent-2.txt & concurrent_pid_2=$!
wait "$concurrent_pid_1" "$concurrent_pid_2"
concurrent_count="$(run_psql -Atqc "SELECT count(*) FROM metric_snapshots WHERE tenant_id='70000000-0000-0000-0000-000000000701' AND period_key='2026-10' AND content_hash=repeat('5',64)")"
[[ "$concurrent_count" == "1" ]] || { echo "Concurrent snapshot requests did not converge to one logical row" >&2; exit 1; }
