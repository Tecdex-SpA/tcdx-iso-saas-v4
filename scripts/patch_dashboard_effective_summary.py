...
backup = p.with_suffix(p.suffix + f".bak_effective_iso_summary_{datetime.now().strftime('%Y%m%d_%H%M%S')}")
...
if "type EffectiveIsoHealthRow" not in text:
    # Insertar después de imports/use client. Buscamos antes del primer type/interface o antes del componente.
    m = re.search(r"(type\s+\w+\s*=|interface\s+\w+\s+|export default function|function Dashboard)", text)
    if not m:
        raise SystemExit("ERROR: no encontré ancla para insertar tipos/helpers.")
    text = text[:m.start()] + type_block + "\n" + text[m.start():]
...
if "effectiveHealthRows" not in text:
    # Insertar después del primer useState existente dentro del componente.
    m = re.search(r"(\s+const\s+\[[^\]]+,\s*set[^\]]+\]\s*=\s*useState[^\n]+;\n)", text)
    if not m:
        raise SystemExit("ERROR: no encontré un useState para insertar estado effectiveHealthRows.")
    text = text[:m.end()] + state_line + "\n" + text[m.end():]

# ============================================================
# 3) Cargar API /api/kpi/effective-health-summary/:tenantId
# ============================================================

load_block = """
  const loadEffectiveHealthSummary = async () => {
...
if "loadEffectiveHealthSummary" not in text:
    # Insertar antes de la primera función handler conocida o antes del return principal.
    m = re.search(r"(\n\s*const\s+handle|\n\s*function\s+handle|\n\s*return\s*\()", text)
    if not m:
        raise SystemExit("ERROR: no encontré ancla para insertar loadEffectiveHealthSummary.")
    text = text[:m.start()] + "\n" + load_block + text[m.start():]

# ============================================================
# 4) Ejecutar carga dentro de useEffect
# ============================================================

if "loadEffectiveHealthSummary();" not in text:
    # Insertar dentro de un useEffect que ya cargue dashboard, si existe.
    use_effects = list(re.finditer(r"useEffect\s*\(\s*\(\)\s*=>\s*\{", text))
    if not use_effects:
        raise SystemExit("ERROR: no encontré useEffect para llamar loadEffectiveHealthSummary.")

    # Usar el primer useEffect que tenga user/token/API o dashboard; si no, el primero.
    chosen = None
    for m in use_effects:
        block = text[m.start():m.start()+900]
        if "user" in block or "dashboard" in block or "load" in block:
            chosen = m
            break
    if chosen is None:
        chosen = use_effects[0]

    insert_at = chosen.end()
    text = text[:insert_at] + "\n    loadEffectiveHealthSummary();" + text[insert_at:]

# ============================================================
# 5) Métricas derivadas para render
# ============================================================
...
if "effectiveActiveRows" not in text:
    # Insertar antes del return principal.
    m = re.search(r"\n\s*return\s*\(", text)
    if not m:
        raise SystemExit("ERROR: no encontré return principal.")
...
if "Salud ISO efectiva" not in text:
    # Insertar lo más arriba posible dentro del contenido principal.
    anchors = [
        "<main",
        "Vista Ejecutiva",
        "Bienvenido",
    ...
    # Intento más seguro: después de cierre de header/sección inicial si existe.
    ...
    if not inserted:
        m = re.search(r"\n\s*return\s*\(\s*<", text)
        if not m:
            raise SystemExit("ERROR: no encontré dónde insertar bloque visual.")
        # No insertamos aquí porque puede romper JSX. Mejor fallar.
        raise SystemExit("ERROR: no encontré <main>. Enviar sed del return principal.")

# ============================================================
# 7) Limpieza: evitar doble inserción o referencias duplicadas básicas
# ============================================================
