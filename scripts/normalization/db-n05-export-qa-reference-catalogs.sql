...
\copy (SELECT ccs.control_id AS qa_control_id, cc.iso AS qa_catalog_standard_code, ccs.standard_code AS mapped_standard_code, cc.clause AS mapped_clause, ccs.is_primary, ccs.created_at, ccs.updated_at FROM public.controls_catalog_standards ccs JOIN public.controls_catalog cc ON cc.id = ccs.control_id WHERE cc.tenant_id IS NULL AND cc.iso IN ('ISO9001', 'ISO27001', 'ISO42001') AND ccs.standard_code IN ('ISO9001', 'ISO27001', 'ISO42001') AND cc.source_type !~* '(^|[^a-z])(demo|test|preview|backup)([^a-z]|$)' ORDER BY cc.iso, ccs.standard_code, cc.clause, ccs.control_id) TO STDOUT WITH CSV HEADER
