...
def _llm_structured_enrichment(payload: Dict[str, Any], base: Dict[str, Any], *, model_mode: str, depth: str, request_id: str, endpoint: str) -> Dict[str, Any]:
    ...
    prompt = {
        ...
        "payload_summary": {
            ...
            "company_profile": payload.get("company_profile") or payload.get("profile") or payload.get("context", {}).get("company_profile"),
            "company_profile_impact": payload.get("company_profile_impact") or payload.get("context", {}).get("company_profile_impact"),
            "company_applicability_universe": payload.get("company_applicability_universe") or payload.get("context", {}).get("company_applicability_universe"),
            "applicable_controls": _as_list(payload.get("applicable_controls") or payload.get("context", {}).get("applicable_controls"), 10),
            "applicable_kpis": _as_list(payload.get("applicable_kpis") or payload.get("context", {}).get("applicable_kpis"), 10),
            "applicable_evidence_requirements": _as_list(payload.get("applicable_evidence_requirements") or payload.get("context", {}).get("applicable_evidence_requirements"), 10),
            "applicability_exclusions_summary": payload.get("applicability_exclusions_summary") or payload.get("context", {}).get("applicability_exclusions_summary"),
        ...
        "rules": [
            ...
            "No recomiendes controles, KPIs o evidencias fuera del company_applicability_universe.",
        ],
    }
    try:
