...
export default function CicloVidaPage() {
  ...
  const summary = useMemo(() => {
    ...
    const healthy = allCards.filter(
      (c) => c.health_status === 'saludable'
    ).length;
  ...
  async function handleDrop(stageCode: string, e: React.DragEvent<HTMLDivElement>) {
    ...
    try {
      ...
      await refreshBoard();
      setSuccessMessage(
        `Movimiento enviado a ${prettifyStage(stageCode, filteredBoard?.stages || [])} y quedó pendiente de validación del auditor.`
      );
      setRequestReason('');
    } catch (err: any) {
  ...
  async function handleReviewRequest(action: 'confirmar' | 'rechazar') {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(
          data?.detail || data?.error || 'No fue posible revisar la solicitud'
        );
      }
      ...
      <div className="mx-auto max-w-[1900px] space-y-6">
        <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          <div className="flex flex-col gap-4">
            <div className="flex flex-col gap-5 xl:flex-row xl:items-start xl:justify-between">
              <div>
                ...
                <p className="mt-2 text-sm text-slate-600">
                  Vista kanban multinorma por operación, con salud, madurez y seguimiento por etapa.
                </p>
              </div>
            ...
            {!topPanelCollapsed && (
              <>
                <div className="grid min-w-[320px] grid-cols-1 gap-3 sm:grid-cols-3">
        ...
        {!loading && filteredBoard ? (
          <div
            className={`grid grid-cols-1 gap-6 ${
              rightPanelCollapsed ? 'xl:grid-cols-[1fr_64px]' : 'xl:grid-cols-[1fr_420px]'
            }`}
          >
            <div className="overflow-x-auto">
              <div className="flex min-w-[1820px] gap-4">
                {filteredBoard.columns.map((column) => {
                  ...
                  return (
                    <div
                      ...
                      className={`w-[385px] flex-shrink-0 rounded-3xl border bg-white shadow-sm transition ${
                    ...
                    >
                      <div
                        className={`rounded-t-3xl px-4 py-4 ${stageHeaderClasses(
                          column.stage_code
                        )}`}
                      >
            ...
            {rightPanelCollapsed ? (
              <aside className="sticky top-0 self-start">
                <div className="rounded-3xl border border-slate-200 bg-white shadow-sm overflow-hidden">
                  ...
                  >
                    ...
                    <span className="text-[11px] font-semibold uppercase tracking-[0.2em] [writing-mode:vertical-rl] rotate-180">
                      Panel
                    </span>
                  </button>
                </div>
              ...
              <aside className="space-y-6">
                      ...
                      {isAuditor ? (
                        <div className="mt-5 flex gap-3">
                          ...
                          <button
                            ...
                            className="flex-1 rounded-xl bg-rose-600 px-4 py-3 text-sm font-semibold text-white transition hover:bg-rose-700 disabled:cursor-not-allowed disabled:opacity-60"
                          >
                            Rechazar
                          </button>
                        </div>
                      ) : (
                        <div className="mt-5 rounded-xl border border-amber-200 bg-amber-50 px-3 py-3 text-sm text-amber-700">
                ...
                <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
                  ...
                  {!selectedCardDetail ? (
                    <div className="mt-5 rounded-2xl border border-dashed border-slate-200 bg-slate-50 p-5 text-sm text-slate-400">
                    ...
                    <div className="mt-5">
                        ...
                        <div className="mt-3 space-y-2 text-sm text-slate-600">
                          ...
                          <div>
                            ...
                            {selectedCardDetail.pending_stage_code
                              ? prettifyStage(selectedCardDetail.pending_stage_code, filteredBoard.stages)
                              : 'No'}
                          </div>
                          <div>
                            <span className="font-semibold">Salud:</span>{' '}
                      ...
                      <div className="mt-4 grid grid-cols-2 gap-3">
                        ...
                        <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                          ...
                          <div className="mt-2 text-lg font-bold text-slate-900">
                            {selectedCardDetail.enabled_controls_count}/{selectedCardDetail.catalog_controls_count}
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="mt-4 grid grid-cols-2 gap-3">
                        <button
                          ...
                          className="rounded-2xl border border-slate-200 bg-white px-4 py-4 text-sm font-semibold text-slate-700 transition hover:border-slate-300 hover:bg-slate-50"
                        >
                          Ir a Controles
                        </button>
                      ...
                      <div className="mt-4 rounded-2xl border border-indigo-200 bg-indigo-50 p-4 text-sm text-indigo-700">
                        Esta tarjeta ya está lista para alimentar análisis narrativos y resúmenes del motor de IA usando su etapa, salud, madurez y eventos de transición.
                      </div>
                    </div>
                  )}
                </div>
