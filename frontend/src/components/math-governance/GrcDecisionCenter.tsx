...
export default function GrcDecisionCenter({
  ...
  const summaryCards = [
    ['Críticos', summary.red, 'text-red-700'],
    ['Atención', summary.amber, 'text-amber-700'],
    ['Controlados', summary.green, 'text-emerald-700'],
    ['Sin medición', summary.gray, 'text-slate-700'],
    ['Medidos', summary.measured, 'text-slate-950'],
  ] as const;
        ...
        {!loading && !error && (
          <div className="mt-5 grid gap-4 xl:grid-cols-[minmax(0,1fr)_minmax(280px,0.42fr)]">
            <div className="grid grid-cols-2 gap-3 md:grid-cols-5">
              {summaryCards.map(([label, value, color]) => (
                <div key={label} className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2">
                  ...
                  <div className={`mt-1 text-lg font-bold ${color}`}>{Number(value)}</div>
                </div>
              ))}
            </div>
            <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-950">
              ...
              {priorities.length === 0 ? (
                <p className="mt-2 text-amber-900">No hay prioridades críticas o en atención con los datos actuales.</p>
              ) : (
                <>
  ...
  return <section className="rounded-[var(--tcdx-radius-tecdex-lg)] border border-[var(--tcdx-color-border)] bg-white p-5 shadow-[var(--tcdx-shadow-tecdex-sm)]">
    ...
    {loading&&<div className="mt-4 rounded-md border border-dashed p-4 text-sm">Cargando decisiones…</div>}
    {error&&<div role="alert" className="mt-4 rounded-md border border-red-200 bg-red-50 p-4 text-sm text-red-950">{error}</div>}
    {!loading&&!error&&<><div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-5">{summaryCards.map(([label,value,color])=><div key={String(label)} className="rounded-lg border border-slate-200 p-3"><div className="text-xs text-slate-600">{label}</div><div className={`mt-1 text-2xl font-bold ${color}`}>{Number(value)}</div></div>)}</div>
