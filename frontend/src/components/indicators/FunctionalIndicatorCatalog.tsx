...
function componentLabel(component:string){const labels:Record<string,string>={completeness:'Completitud',accuracy:'Exactitud',consistency:'Consistencia',freshness:'Freshness',lineage:'Lineage',validation:'Validación',stability:'Estabilidad',coverage:'Cobertura',dataTrust:'Data Trust',risk:'Riesgo residual',compliance:'Cumplimiento',actions:'Planes de acción',evidence:'Evidencia vigente'};return labels[component]||component}
...
export default function FunctionalIndicatorCatalog({ metricCode }: { metricCode?:string }){
  ...
  return <div className="space-y-5">
    ...
    {!metricCode&&<div className="grid gap-4 lg:grid-cols-2">{visible.map((item)=>{const state=actionable(item.latest_snapshot);return <article key={item.definition.code} className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-[0.16em] text-blue-700">{item.definition.domain}</p><h2 className="mt-1 text-lg font-bold text-slate-950">{item.definition.name}</h2></div><UniversalStateBadge state={universalSnapshotState(item.latest_snapshot)} label={item.latest_snapshot?.interpretation?.classification?.label||undefined} /></div><p className="mt-3 text-sm leading-6 text-slate-600">{item.definition.definition}</p><div className="mt-4 grid gap-2 text-sm sm:grid-cols-3"><div><span className="block text-xs text-slate-500">Resultado</span><strong>{formatValue(item.latest_snapshot)}</strong></div><div><span className="block text-xs text-slate-500">Cobertura</span><strong>{pct(item.latest_snapshot?.coverage)}</strong></div><DataTrustIndicator status={item.latest_snapshot?.trust?.status || null} confidence={item.latest_snapshot?.trust?.score ?? null} coverage={item.latest_snapshot?.coverage ?? null} freshness={item.latest_snapshot?.freshness?.status || null} label="Data Trust del snapshot" /></div>{state&&<p className="mt-3 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs leading-5 text-amber-950">{state.why} {state.missing_components?.[0]?.label?`Falta: ${state.missing_components[0].label}.`:''}</p>}<Link href={`/metricas/${encodeURIComponent(item.definition.code)}`} className="mt-4 inline-flex min-h-10 items-center rounded-lg border border-blue-200 px-3 text-sm font-semibold text-blue-800 hover:bg-blue-50">Abrir indicador</Link></article>})}</div>}
    {selected&&<article className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm"><div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between"><div><p className="text-xs font-bold uppercase tracking-[0.16em] text-blue-700">{selected.definition.domain}</p><h1 className="mt-1 text-2xl font-bold text-slate-950">{selected.definition.name}</h1><p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">{selected.definition.definition}</p></div><UniversalStateBadge className="self-start" state={universalSnapshotState(selected.latest_snapshot)} label={selected.latest_snapshot?.interpretation?.classification?.label||undefined} /></div>
...
function ActionableStatePanel({state}:{state:ActionableState|null}){if(!state||state.state==='calculated')return null;const missing=state.missing_components||[];return <section className="mt-6 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-950"><div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between"><div><h2 className="font-bold">{state.label||'No calculable'}</h2><p className="mt-1 leading-6">{state.why||'El indicador requiere evidencia adicional para calcularse.'}</p></div>{state.route_to_fix&&<Link href={state.route_to_fix} className="inline-flex min-h-10 shrink-0 items-center justify-center rounded-lg border border-amber-300 bg-white px-3 text-xs font-bold text-amber-900 hover:bg-amber-100">Resolver origen</Link>}</div>{missing.length>0&&<div className="mt-4"><p className="text-xs font-bold uppercase tracking-wide text-amber-800">Qué falta</p><ul className="mt-2 grid gap-2 md:grid-cols-2">{missing.map((item)=><li key={item.component} className="rounded-lg border border-amber-200 bg-white px-3 py-2"><div className="font-semibold">{item.label||item.component}</div><div className="mt-1 text-xs leading-5 text-amber-900">{item.reason||item.rule||'Componente requerido por contrato oficial.'}</div>{item.route_to_fix&&<Link href={item.route_to_fix} className="mt-2 inline-flex text-xs font-bold text-blue-800 underline-offset-2 hover:underline">{item.action||'Abrir origen'}</Link>}</li>)}</ul></div>}<p className="mt-3 text-xs leading-5 text-amber-900">{state.expected_after_resolution||'Luego de completar la evidencia, recalcula el indicador desde el pipeline oficial.'}</p>{state.required_capability&&<p className="mt-1 text-xs text-amber-800">Capability requerida: {state.required_capability}</p>}</section>}
