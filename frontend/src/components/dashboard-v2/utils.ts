...
export function statusLabel(value?: string | null) {
  const labels: Record<string, string> = {
    ...
    atencion: 'Atencion',
    critico: 'Critico',
    acciones_abiertas: 'Acciones abiertas',
    sin_bloqueos_operativos: 'Sin bloqueos operativos',
    partial: 'Parcial',
    limited: 'Limitado',
    complete: 'Completo',
    prepared: 'Preparado',
  ...
  return labels[String(value || '').toLowerCase()] || String(value || 'Sin datos suficientes');
}

export function scoreClass(score: number | string | null | undefined) {
...
export function chipClass(value?: string | null) {
  const normalized = String(value || '').toLowerCase();
  if (['saludable', 'estable', 'listo', 'ready', 'complete'].includes(normalized)) {
    return 'border-emerald-200 bg-emerald-50 text-emerald-800';
  }
  if (['atencion', 'attention', 'partial', 'en_progreso'].includes(normalized)) {
    return 'border-amber-200 bg-amber-50 text-amber-800';
  }
  if (['transicion', 'prepared'].includes(normalized)) {
    return 'border-sky-200 bg-sky-50 text-sky-800';
  }
  if (['critico', 'limited', 'requiere_atencion'].includes(normalized)) {
