...
async function zoom(page:Page,ratio:number){await page.evaluate((z)=>{document.documentElement.style.zoom=String(z)},ratio)}
async function open(page:Page,route:string,ratio=0.5){await page.goto(route,{waitUntil:'domcontentloaded'});await page.waitForLoadState('networkidle').catch(()=>{});await page.waitForTimeout(1200);await zoom(page,ratio)}
async function shot(page:Page,key:string,name:string){const d=path.join(out,key,'before-after');fs.mkdirSync(d,{recursive:true});await page.screenshot({path:path.join(d,`${name}.png`),fullPage:true})}
...
for(const s of scenarios){test(`${s.key} measurable ingestion before-after`,async({page})=>{const api=await createRequest.newContext({baseURL:apiBase});try{const{token,tenantId}=await login(api,s.email);await session(page,token);const before=await countState(api,token,tenantId,s.iso);
  await open(page,'/dashboard?view=executive',0.5);await shot(page,s.key,'01-dashboard-ANTES-ingesta');
  await open(page,'/hallazgos',0.75);await shot(page,s.key,'02-hallazgos-ANTES-ingesta');
  ...
  const existing=rows(await req(api,'GET',`/api/findings/${tenantId}?iso=${s.iso}`,token));
  const definitions=[
    ['001','alta','Brecha crítica de evidencia objetiva'],['002','alta','Desviación mayor de operación'],['003','media','Observación de trazabilidad'],['004','media','Seguimiento documental pendiente'],['005','baja','Oportunidad de mejora'],['006','baja','Mejora de consistencia documental']
  ];
  for(const [n,severity,label] of definitions){const code=`${s.prefix}-${n}`;let f=existing.find((x:any)=>String(x.title||'').includes(code));if(!f){const created=await req(api,'POST','/api/findings',token,{tenant_id:tenantId,iso_code:s.iso,title:`${code} ${label}`,description:`${code}. Registro demo medible para poblar dashboard, tendencias y carga operativa.`,finding_type:'observacion',severity,status:'abierto',source_type:'manual',owner:s.email,detected_by:'Dataset documental medible',due_date:new Date(Date.now()+(10+Number(n))*86400000).toISOString().slice(0,10),tenant_control_id:tcId});f=created.data||created.finding||created}if(f?.id){try{await req(api,'POST',`/api/findings/${f.id}/create-action`,token,{})}catch{}}}

  const audits=rows(await req(api,'GET',`/api/audits/${tenantId}?iso=${s.iso}`,token));for(const n of ['002','003']){const requester=`${s.prefix}-AUD-${n}`;if(!audits.some((x:any)=>String(x.requester_name||'')===requester)){await req(api,'POST','/api/audits',token,{tenant_id:tenantId,iso:s.iso,start_date:new Date(Date.now()+Number(n)*86400000).toISOString().slice(0,10),end_date:new Date(Date.now()+(7+Number(n))*86400000).toISOString().slice(0,10),requester_name:requester,auditor_type:'interno',auditor_name:s.auditor})}}

  const after=await countState(api,token,tenantId,s.iso);expect(after.findings).toBeGreaterThanOrEqual(before.findings);expect(after.audits).toBeGreaterThanOrEqual(before.audits);
  await open(page,'/hallazgos',0.75);await shot(page,s.key,'03-hallazgos-DESPUES-ingesta');
  await open(page,'/planes-accion',0.75);await shot(page,s.key,'04-planes-DESPUES-ingesta');
  await open(page,'/auditorias',0.75);await shot(page,s.key,'05-auditorias-DESPUES-ingesta');
  await open(page,'/dashboard?view=executive',0.5);await shot(page,s.key,'06-dashboard-DESPUES-ingesta');
  await open(page,'/dashboard?view=kpi',0.5);await shot(page,s.key,'07-dashboard-kpi-DESPUES-ingesta');
  await open(page,'/dashboard?view=iso',0.5);await shot(page,s.key,'08-dashboard-salud-DESPUES-ingesta');
  await open(page,'/iso-health',0.5);await shot(page,s.key,'09-iso-health-DESPUES-ingesta');
  fs.mkdirSync(path.join(root,'artifacts/documentation'),{recursive:true});fs.appendFileSync(path.join(root,'artifacts/documentation/demo-enrichment.ndjson'),JSON.stringify({scenario:s.key,tenantId,iso:s.iso,before,after,delta:{findings:after.findings-before.findings,openFindings:after.openFindings-before.openFindings,audits:after.audits-before.audits,completedAudits:after.completedAudits-before.completedAudits}})+'\n');
