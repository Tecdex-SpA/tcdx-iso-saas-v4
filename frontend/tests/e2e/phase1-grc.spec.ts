...
test.describe('Phase 1 GRC runtime', () => {
  ...
  test('persistencia web tras recarga conserva inicialización operacional', async ({ page }) => {
    const persistedStatus = await requireApi().get('/api/grc/bootstrap/status', { headers: { Authorization: `Bearer ${adminToken}` } });
    const persistedBody = await persistedStatus.json().catch(() => ({}));
