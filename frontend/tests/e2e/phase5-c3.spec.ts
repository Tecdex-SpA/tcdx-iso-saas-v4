...
for(const profile of profiles){
  test(`${profile.name}: API, UI, RBAC y snapshot son consistentes`,async({page})=>{
    ...
    if(profile.role==='admin'){catalogRequests=0;moduleRequests=0;await page.goto('/dashboard');await expect(page.getByRole('heading',{name:'Decisiones GRC derivadas del dashboard'})).toBeVisible();await expect(page.getByText(profile.value===82?'82 %':'64 %',{exact:true}).first()).toBeVisible();expect(catalogRequests).toBeLessThanOrEqual(1);expect(moduleRequests).toBeLessThanOrEqual(1);}
