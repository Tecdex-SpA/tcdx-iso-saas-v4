...
const routes = JSON.parse(
  fs.readFileSync(path.join(root, 'scripts/demo/demo-visual-routes.json'), 'utf8'),
) as RouteItem[];
const uniqueRoutes = routes.filter(
  (item, index) => routes.findIndex((candidate) => candidate.route === item.route) === index,
);
...
function resolve(value: string) {
  return value.replace(/:tenant/g, tenantId).replace(/:metric1/g, demoUuid('metric-1'));
}

function arrays(value: unknown, output: unknown[][] = []): unknown[][] {
  if (Array.isArray(value)) {
    output.push(value);
    value.forEach((item) => arrays(item, output));
  } else if (value && typeof value === 'object') {
    Object.values(value).forEach((item) => arrays(item, output));
  }
  return output;
}
...
function scalar(value: unknown): string | null {
  if (Array.isArray(value)) {
    for (const item of value) {
      const found = scalar(item);
      if (found) return found;
    }
  } else if (value && typeof value === 'object') {
    for (const [key, item] of Object.entries(value)) {
      if (!/id|tenant|hash|metadata/i.test(key)) {
        const found = scalar(item);
        if (found) return found;
      }
    }
  } else if ((typeof value === 'string' && value.length >= 3) || (typeof value === 'number' && value > 0)) {
    return String(value);
  }
  return null;
}
...
async function validateApi(api: APIRequestContext, token: string, item: RouteItem) {
  const response = await api.get(resolve(item.endpoint), {
  ...
  expect(
    longest.length >= item.minimum || positive.length > 0,
    `${item.endpoint} must expose non-zero data`,
  ).toBeTruthy();
  const seriesMinimum = item.seriesMinimum;
  if (seriesMinimum) {
    expect(found.some((items) => items.length >= seriesMinimum), `${item.endpoint} series`).toBeTruthy();
  }
  expect(JSON.stringify(data)).not.toMatch(new RegExp(`tenant_id"\\s*:\\s*"(?!${tenantId})`));
  return {
    data,
    observed: Math.max(longest.length, positive.length ? 1 : 0),
    representative: scalar(data),
  };
}

test.beforeAll(() => {
  expect(apiBase, 'DEMO_API_BASE_URL is required').toBeTruthy();
  expect(password, 'DEMO_ADMIN_PASSWORD is required').toBeTruthy();
...
for (const account of [
  { role: 'admin', email: 'admin.demo@tcdx.demo' },
  { role: 'auditor', email: 'auditor.demo@tcdx.demo' },
]) {
  test.describe(`${account.role} visual coverage`, () => {
    ...
    test.beforeAll(async () => {
      api = await createRequest.newContext({ baseURL: apiBase });
      token = await login(api, account.email);
    ...
    for (const item of uniqueRoutes) {
      test(`${account.role} ${item.route}`, async ({ page }) => {
        ...
        for (const endpointItem of endpointItems) {
          apiEvidence.push(await validateApi(api, token, endpointItem));
        }
        ...
        await expect(page.locator('body')).not.toContainText(
          /capability.+(bloqueada|no habilitada)|error boundary|application error|internal server error/i,
        );
        ...
        if (endpointItems.some((value) => ['gráfico', 'heatmap'].includes(value.type))) {
          const marks = await page
            .locator('svg path, svg rect, canvas, [data-chart], [data-heatmap-cell]')
            .count();
        ...
        if (screenshotRoutes.has(cleanRoute)) {
          ...
          await page.screenshot({
            path: path.join(evidenceDir, `${slug}-${account.role}.png`),
            fullPage: true,
          });
        }

        if (cleanRoute === '/diagnostico') {
          const standardSelect = page
            .locator('select:has(option[value="ISO9001"]):has(option[value="ISO27001"])')
            .first();
          ...
          for (const standard of ['ISO9001', 'ISO27001']) {
            ...
            await expect(page.locator('body')).toContainText(
              new RegExp(standard.replace('ISO', 'ISO\\s*'), 'i'),
            );
            await page.screenshot({
              path: path.join(
                evidenceDir,
                `${standard === 'ISO9001' ? 'iso-9001' : 'iso-27001'}-${account.role}.png`,
              ),
              fullPage: true,
            });
          }
