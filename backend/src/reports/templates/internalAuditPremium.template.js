...
function renderCoverPage(data) {
  ...
  return `
    <div class="pageTitleBlock">
      ...
      <h2>Alcance, criterios y conclusión</h2>
      <p>Informe formal de auditoría interna con alcance, criterios, hallazgos, evidencias y plan de cierre.</p>
    </div>

    <section class="summaryHero">
      <div>
        <div class="reportBadge">Auditoría / Verificación</div>
        <h3>${escapeHtml(standard.displayName)}</h3>
        <p>Evaluación formal de conformidad, hallazgos, no conformidades, evidencias y recomendaciones de cierre.</p>
        <div class="badgeRow">
...
function renderAuditFindingsPage(data) {
  return `
    <div class="pageTitleBlock">
      <span>ROADMAP</span>
...
function renderAuditFindingsPage(data) {
  const stats = getStats(data);
  const standard = getStandardContext(data);
  const findings = toNumber(stats.findings.open_findings, 0);
  const pendingEvidence = toNumber(standard.metrics?.pending_evidence_count || stats.evidences.pending_evidences, 0);
  const expiredEvidence = toNumber(standard.metrics?.expired_evidence_count || stats.evidences.expired_evidences, 0);

  const rows = [
    {
      tipo: 'Hallazgos abiertos',
      cantidad: findings,
      criterio: 'Seguimiento y cierre',
      lectura: findings > 0 ? 'Requiere análisis de causa y acción correctiva.' : 'Sin hallazgos abiertos relevantes.',
    },
    {
      tipo: 'Evidencias pendientes',
      cantidad: pendingEvidence,
      criterio: 'Evidencia objetiva',
      lectura: pendingEvidence > 0 ? 'Limita trazabilidad auditable.' : 'No se observan pendientes principales.',
    },
    {
      tipo: 'Evidencias vencidas',
      cantidad: expiredEvidence,
      criterio: 'Vigencia documental',
      lectura: expiredEvidence > 0 ? 'Debe renovarse evidencia antes de cierre.' : 'Sin vencimientos críticos visibles.',
    },
  ];
  ...
  return `
    <div class="pageTitleBlock">
      <span>HALLAZGOS</span>
      <h2>Hallazgos, evidencias y no conformidades</h2>
      <p>Resumen formal para seguimiento de auditoría interna.</p>
    </div>
    ...
    ${renderPremiumTable(
      [
        ...
        { key: 'lectura', label: 'Lectura auditora' },
      ],
      rows,
      { title: 'Resumen de hallazgos y evidencias' }
    )}

    <section class="noteBox">
      <strong>Criterio auditor:</strong>
      <p>Todo hallazgo debe vincularse a evidencia objetiva, requisito aplicable, responsable y acción de cierre.</p>
    </section>
  `;
}
...
function renderAuditConclusionPage(data) {
  const standard = getStandardContext(data);
  const stats = getStats(data);
  const score = toNumber(standard.metrics?.avg_health_score || stats.controls.average_score, 0);
  const findings = toNumber(stats.findings.open_findings, 0);
  const expiredEvidence = toNumber(standard.metrics?.expired_evidence_count || stats.evidences.expired_evidences, 0);

  const conclusion =
    score >= 80 && findings === 0 && expiredEvidence === 0
      ? 'El sistema presenta condiciones favorables para sostener conformidad, sujeto a revisión de evidencia final.'
      : score >= 60
        ? 'El sistema presenta conformidad parcial o controlada, con brechas que deben cerrarse antes de una auditoría externa.'
        : 'El sistema requiere remediación prioritaria antes de ser considerado preparado para auditoría formal.';

  return `
    <div class="pageTitleBlock">
      <span>CONCLUSIÓN</span>
      <h2>Conclusión de auditoría interna</h2>
      <p>Lectura final orientada a decisión de cierre, seguimiento o remediación.</p>
    </div>

    <section class="summaryHero">
      <div>
        <div class="reportBadge">Conclusión auditora</div>
        <h3>${escapeHtml(standard.displayName)}</h3>
        <p>${escapeHtml(conclusion)}</p>
        <div class="badgeRow">
          ${renderBadge(score >= 80 ? 'Condición favorable' : score >= 60 ? 'Con observaciones' : 'Requiere remediación', score >= 80 ? 'success' : score >= 60 ? 'warning' : 'danger')}
          ${renderBadge('No reemplaza certificación externa', 'neutral')}
        </div>
      </div>
      <div class="scoreBox">
        <span>Score auditado</span>
        <strong>${escapeHtml(Math.round(score * 10) / 10)}%</strong>
        <em>base interna</em>
      </div>
    </section>

    <section class="noteBox">
      <strong>Plan de cierre:</strong>
      <ul>
        <li>Formalizar hallazgos con requisito, evidencia, causa y acción correctiva.</li>
        <li>Asignar responsable y fecha de cierre a cada desviación.</li>
        <li>Revisar evidencia de cierre antes de pasar a verificación final.</li>
      </ul>
    </section>
  `;
}

function renderInternalAuditPremiumTemplate(data = {}) {
  const tenant = data.tenant || {};
  const title = 'Informe de Auditoría Interna';
  const generatedAt = data.generated_at || data.generatedAt || new Date();

  const pages = [
    renderCoverPage(data),
    renderAuditFindingsPage(data),
    renderAuditConclusionPage(data),
  ];
...
module.exports = {
  renderInternalAuditPremiumTemplate,
};
