...
function cleanAiPdfText(value, fallback = '') {
...
function normalizeAiStandardCode(value) {
  const raw = cleanAiPdfText(value)
    .toUpperCase()
    .replace(/\s+/g, '')
...
function getAiAuditorIsoProfile(standardCode, versionCode = '') {
  const code = normalizeAiStandardCode(standardCode);

  if (code === 'ISO27001') {
    return {
      code: 'ISO27001',
      label: `ISO/IEC 27001${versionCode ? `:${versionCode}` : ':2022'}`,
      system: 'Sistema de Gestión de Seguridad de la Información',
      question: '¿El SGSI controla adecuadamente riesgos de confidencialidad, integridad y disponibilidad?',
      criteria: [
        'Alcance del SGSI',
      ...
      evidenceFocus: [
        ...
        'Evidencias de controles Anexo A',
        'Registros de acceso',
        'Continuidad, backup e incidentes',
      ],
      auditorFindings: [
        'Evidencia técnica insuficiente para controles Anexo A.',
        'Riesgo residual sin tratamiento documentado.',
        'Activo crítico sin propietario o sin evidencia vigente.',
      ],
      auditorActions: [
        'Actualizar SoA y vincular evidencia por control aplicable.',
        'Revisar matriz de riesgos y tratamientos pendientes.',
        'Formalizar propietarios de activos críticos y evidencias técnicas.',
      ],
    };
  }
  ...
  if (code === 'ISO9001') {
    return {
      code: 'ISO9001',
      label: `ISO 9001${versionCode ? `:${versionCode}` : ':2015'}`,
      system: 'Sistema de Gestión de la Calidad',
      question: '¿El SGC demuestra control de procesos, satisfacción del cliente, no conformidades y mejora continua?',
      criteria: [
      ...
      evidenceFocus: [
        'Mapa/procedimientos de procesos',
      ...
      auditorFindings: [
        'Evidencia insuficiente de seguimiento de objetivos de calidad.',
        'Proceso crítico sin medición o evidencia suficiente.',
        'No conformidad o acción correctiva sin cierre verificable.',
      ],
      auditorActions: [
        'Actualizar objetivos de calidad, indicadores y seguimiento.',
        'Cerrar no conformidades con causa raíz y evidencia de eficacia.',
        'Formalizar responsables por proceso y registros de operación.',
      ],
    };
  }
  ...
  return {
    code,
    label: cleanAiPdfText(standardCode || 'Norma ISO'),
    system: 'Sistema de Gestión',
    question: '¿El sistema de gestión demuestra cumplimiento verificable frente a requisitos aplicables?',
    criteria: [
      'Alcance del sistema',
    ...
    auditorFindings: [
      'Evidencia insuficiente para demostrar cumplimiento.',
      'Control sin responsable o sin seguimiento documentado.',
    ],
    auditorActions: [
      'Completar evidencia objetiva por control.',
      'Asignar responsable, plazo y evidencia de cierre.',
    ],
  };
}

function getAiAuditAnalysisPayload(source = {}) {
  const full = source.full_result_json || source.full_result || source.result || source.analysis || source || {};
  const summary = source.summary_json || full.summary || full.summary_json || {};
  const coverage = source.coverage_json || full.coverage || full.coverage_json || {};
  const suggestions = source.suggestions_json || full.suggestions || full.suggestions_json || {};
  const trace = source.trace_json || full.trace || full.trace_json || {};
  ...
  const standardCode =
    ...
    full.standard_code ||
    full.iso ||
    summary.standard_code ||
    coverage.standard_code ||
    'ISO9001';
  ...
  const profile = getAiAuditorIsoProfile(standardCode, versionCode);

  const controlsReviewed = Number(
    coverage.controls_reviewed ||
      coverage.reviewed_controls ||
      full.controls_reviewed ||
      summary.controls_reviewed ||
      0
  );

  const evidencesReviewed = Number(
    coverage.evidences_reviewed ||
      coverage.evidence_reviewed ||
      full.evidences_reviewed ||
      summary.evidences_reviewed ||
      0
  );

  const findingsReviewed = Number(
    coverage.findings_reviewed ||
      full.findings_reviewed ||
      summary.findings_reviewed ||
      0
  );

  const actionsReviewed = Number(
    coverage.actions_reviewed ||
      full.actions_reviewed ||
      summary.actions_reviewed ||
      0
  );
  ...
  const controlsWithAlert = Number(
    coverage.controls_with_issues ||
      coverage.controls_attention ||
      full.controls_with_issues ||
      0
  );

  const evidencesWithAlert = Number(
    coverage.evidence_issues ||
      coverage.evidences_pending ||
      coverage.expired_evidences ||
      full.evidence_issues ||
      0
  );

  const hasWeakSignals =
    controlsWithAlert > 0 ||
    evidencesWithAlert > 0 ||
    Number(coverage.pending_evidences || 0) > 0 ||
    Number(coverage.expired_evidences || 0) > 0 ||
    Number(coverage.open_actions || 0) > 0;

  const evidenceRatio =
    controlsReviewed > 0
      ? Math.min(100, Math.round((evidencesReviewed / controlsReviewed) * 1000) / 10)
      : evidencesReviewed > 0
        ? 60
        : 35;
  ...
  const confidenceScore = Number(
    full.confidence_score ||
      ...
      Math.max(35, Math.min(92, Math.round((evidenceRatio + (controlsReviewed > 0 ? 70 : 40)) / 2)))
  );
  ...
  const auditScore = Math.max(
    ...
    Math.min(
      100,
      rawScore >= 98 && (hasWeakSignals || confidenceScore < 70)
        ? Math.min(84, rawScore)
        : rawScore || Math.round((confidenceScore + evidenceRatio) / 2)
    )
  );

  const readiness =
    auditScore >= 85 && confidenceScore >= 70
      ? {
          label: 'Apto con revisión final',
          severity: 'success',
          conclusion: 'La revisión asistida por IA muestra preparación favorable, sujeta a validación humana de evidencias críticas.',
        }
      : auditScore >= 65
        ? {
            label: 'Apto con observaciones',
            severity: 'warning',
            conclusion: 'El sistema presenta preparación parcial o controlada, con brechas que deben cerrarse antes de auditoría formal.',
          }
        : {
            label: 'No apto todavía',
            severity: 'danger',
            conclusion: 'El sistema requiere remediación prioritaria antes de considerarse preparado para auditoría formal.',
          };
...
function aiSeverityColor(severity) {
  const raw = String(severity || '').toLowerCase();

  if (raw.includes('success') || raw.includes('baja') || raw.includes('ok')) return '#16A34A';
  if (raw.includes('danger') || raw.includes('alta') || raw.includes('crit')) return '#DC2626';
  if (raw.includes('warning') || raw.includes('media')) return '#D97706';

  return '#334155';
}

function addAiPremiumHeader(doc, { title, tenantName, dateText, pageNumber, totalPages }) {
  doc.save();
  doc.rect(0, 0, doc.page.width, 62).fill('#0B2F4F');

  doc.fillColor('#FFFFFF')
    .fontSize(9)
    .font('Helvetica-Bold')
    .text('TCDX by Tecdex', 36, 18);
  ...
  doc.fontSize(8)
    .font('Helvetica')
    .fillColor('#DDE7F3')
    .text(`Fecha de emisión: ${dateText}`, 160, 36, {
      width: doc.page.width - 320,
      align: 'center',
    });
  ...
  doc.fontSize(8)
    .font('Helvetica-Bold')
    .fillColor('#FFFFFF')
    .text(tenantName, doc.page.width - 170, 18, {
      width: 130,
      align: 'right',
    });
  ...
  doc.save();
  doc.rect(0, doc.page.height - 32, doc.page.width, 32).fill('#0B2F4F');

  doc.fillColor('#FFFFFF')
    .fontSize(7)
    .font('Helvetica')
    .text(
      'Documento confidencial · Generado por TCDX by Tecdex · Revisión humana requerida',
      36,
      doc.page.height - 22,
  ...
  doc.font('Helvetica-Bold')
    .text(`Página ${pageNumber} de ${totalPages}`, doc.page.width - 100, doc.page.height - 22, {
      width: 64,
      align: 'right',
    });
...
function addAiPremiumSectionTitle(doc, kicker, title, subtitle = '') {
  doc.roundedRect(36, doc.y, 120, 18, 9).fill('#EEF2FF');

  doc.fillColor('#4338CA')
    .fontSize(7)
    .font('Helvetica-Bold')
    .text(kicker.toUpperCase(), 46, doc.y + 5, {
      width: 100,
      align: 'center',
    });
...
function addAiPremiumCard(doc, x, y, w, h, title, value, helper, color = '#0B2F4F') {
  doc.save();
  ...
  doc.fillColor('#64748B')
    .fontSize(7)
    .font('Helvetica-Bold')
    .text(cleanAiPdfText(title).toUpperCase(), x + 10, y + 10, {
      width: w - 20,
    });
  ...
  doc.fillColor(color)
    .fontSize(18)
    .font('Helvetica-Bold')
    .text(cleanAiPdfText(value), x + 10, y + 26, {
      width: w - 20,
    });
  ...
  doc.fillColor('#64748B')
    .fontSize(7)
    .font('Helvetica')
    .text(cleanAiPdfText(helper), x + 10, y + 50, {
      width: w - 20,
    });
...
function addAiPremiumTable(doc, title, columns, rows, options = {}) {
  const x = 36;
  const width = doc.page.width - 72;
  const headerH = 22;
  const rowH = options.rowHeight || 42;
  ...
  doc.roundedRect(x, y, width, headerH, 8).fill('#EAF2F8');

  let colX = x;

  columns.forEach((col) => {
    const colW = width * col.w;

    doc.fillColor('#0B2F4F')
      .fontSize(7)
      .font('Helvetica-Bold')
      .text(col.label, colX + 6, y + 7, {
        width: colW - 10,
      });
  ...
  rows.forEach((row, idx) => {
    ...
    columns.forEach((col) => {
      ...
      doc.fillColor('#334155')
        .fontSize(7)
        .font('Helvetica')
        .text(cleanAiPdfText(row[col.key]), cellX + 6, y + 7, {
          width: colW - 10,
          height: rowH - 10,
          ellipsis: true,
        });
...
function buildAiPremiumControlRows(payload) {
  return payload.profile.criteria.slice(0, 8).map((criterion, index) => ({
    id: `C-${String(index + 1).padStart(2, '0')}`,
    criterio: criterion,
    control: `Revisión de ${criterion}`,
    estado: index < 3 ? 'Requiere revisión' : 'Sin observación crítica',
    evidencia: index < 3 ? 'Validar suficiencia' : 'Muestra no concluyente',
    lectura: index < 3
      ? 'Debe verificarse evidencia objetiva, responsable, vigencia y trazabilidad.'
      : 'Sin desviación crítica detectada con los datos disponibles.',
  }));
}

function buildAiPremiumGapRows(payload) {
  const rows = [];

  if (payload.confidenceScore < 70) {
    rows.push({
      id: 'B-01',
      severidad: 'alta',
      brecha: 'Confianza IA limitada por suficiencia o calidad de datos',
      evidencia: 'La confianza del análisis no alcanza un nivel alto.',
      accion: 'Validar manualmente evidencias críticas y controles en atención.',
    });
  }

  if (payload.evidencesReviewed < payload.controlsReviewed) {
    rows.push({
      id: 'B-02',
      severidad: 'media',
      brecha: 'Cobertura documental insuficiente frente a controles revisados',
      evidencia: `${payload.evidencesReviewed} evidencias revisadas para ${payload.controlsReviewed} controles.`,
      accion: 'Completar evidencia objetiva por control y requisito aplicable.',
    });
  }
  ...
  rows.push({
    id: 'B-03',
    severidad: 'media',
    brecha: `Validación normativa específica para ${payload.profile.label}`,
    evidencia: payload.profile.question,
    accion: payload.profile.auditorActions[0],
  });

  return rows;
}

function renderAiAuditorPremiumPdfKit(doc, rawData = {}, options = {}) {
  const payload = getAiAuditAnalysisPayload(rawData);

  const tenantName = cleanAiPdfText(
    options.tenantName ||
      rawData.tenant?.name ||
      rawData.tenant_name ||
  ...
  const dateText = cleanAiPdfText(
    options.dateText ||
      new Date(rawData.generated_at || rawData.created_at || Date.now()).toLocaleString('es-CL')
  );

  const title = options.title || 'Informe IA Auditor Senior';
  const totalPages = 5;

  addAiPremiumHeader(doc, { title, tenantName, dateText, pageNumber: 1, totalPages });

  addAiPremiumSectionTitle(
    doc,
    'IA Auditor Senior',
    'Informe auditor asistido por IA',
    'Evaluación no destructiva orientada a auditoría, preparación, evidencia objetiva y revisión humana.'
  );

  const readinessColor = aiSeverityColor(payload.readiness.severity);

  doc.roundedRect(36, doc.y, doc.page.width - 72, 92, 16)
    .fill('#FFFFFF')
    .stroke('#DCE7F3');
  ...
  doc.fillColor('#4338CA')
    .fontSize(8)
    .font('Helvetica-Bold')
    .text('EVALUACIÓN AUDITORA ASISTIDA', 52, doc.y + 14);

  doc.fillColor('#0F172A')
    .fontSize(18)
    .font('Helvetica-Bold')
    .text(payload.profile.label, 52, doc.y + 32, { width: 310 });

  doc.fillColor('#64748B')
    .fontSize(9)
    .font('Helvetica')
    .text(payload.profile.system, 52, doc.y + 56, { width: 320 });

  doc.fillColor('#334155')
    .fontSize(8)
    .font('Helvetica-Bold')
    .text(payload.profile.question, 52, doc.y + 74, { width: 330 });

  doc.roundedRect(doc.page.width - 170, doc.y + 10, 112, 70, 14)
    .fill('#F8FAFC')
    .stroke('#CBD5E1');
  ...
  doc.fillColor('#64748B')
    .fontSize(8)
    .font('Helvetica-Bold')
    .text('RESULTADO IA', doc.page.width - 154, doc.y + 24);

  doc.fillColor(readinessColor)
    .fontSize(25)
    .font('Helvetica-Bold')
    .text(`${Math.round(payload.auditScore * 10) / 10}%`, doc.page.width - 154, doc.y + 43);

  doc.fillColor('#64748B')
    .fontSize(7)
    .font('Helvetica')
    .text(payload.readiness.label, doc.page.width - 154, doc.y + 72, { width: 80 });

  doc.y += 115;

  const cardY = doc.y;
  const cardW = (doc.page.width - 72 - 18) / 4;

  [
    ['Score auditor', `${Math.round(payload.auditScore * 10) / 10}%`, payload.profile.label, readinessColor],
    ['Confianza IA', `${Math.round(payload.confidenceScore * 10) / 10}%`, 'Calidad de datos', payload.confidenceScore >= 70 ? '#16A34A' : '#D97706'],
    ['Controles revisados', payload.controlsReviewed, 'Muestra evaluada', '#0B2F4F'],
    ['Evidencias revisadas', payload.evidencesReviewed, 'Base objetiva', '#0B2F4F'],
    ['Controles con alerta', payload.controlsWithAlert, 'Requieren revisión', payload.controlsWithAlert > 0 ? '#D97706' : '#16A34A'],
    ['Evidencias con alerta', payload.evidencesWithAlert, 'Suficiencia/vigencia', payload.evidencesWithAlert > 0 ? '#DC2626' : '#16A34A'],
    ['Hallazgos revisados', payload.findingsReviewed, 'Entrada auditora', '#0B2F4F'],
    ['Acciones revisadas', payload.actionsReviewed, 'Seguimiento', '#0B2F4F'],
  ].forEach((card, index) => {
  ...
  doc.addPage();
  addAiPremiumHeader(doc, { title, tenantName, dateText, pageNumber: 2, totalPages });
  addAiPremiumSectionTitle(doc, 'Alcance y método', 'Alcance, criterio y metodología', 'Marco usado para interpretar los datos del sistema.');

  addAiPremiumTable(
    doc,
    `Criterios aplicables · ${payload.profile.label}`,
    [
      { key: 'criterio', label: 'Criterio normativo', w: 0.28 },
      { key: 'uso', label: 'Uso auditor', w: 0.34 },
      { key: 'evidencia', label: 'Evidencia esperada', w: 0.38 },
    ],
    payload.profile.criteria.map((criterion, index) => ({
      criterio: criterion,
      uso: index < 3 ? 'Criterio principal de revisión auditora.' : 'Criterio complementario de evaluación y mejora.',
      evidencia: payload.profile.evidenceFocus[index] || 'Evidencia objetiva aplicable.',
    })),
    { rowHeight: 36 }
  );

  doc.addPage();
  addAiPremiumHeader(doc, { title, tenantName, dateText, pageNumber: 3, totalPages });
  addAiPremiumSectionTitle(doc, 'Muestra auditada', 'Controles y evidencias revisadas', 'Muestra representativa usada por IA Auditor.');

  addAiPremiumTable(
    doc,
    'Controles priorizados',
    [
      { key: 'id', label: 'ID', w: 0.08 },
      { key: 'criterio', label: 'Criterio', w: 0.18 },
      { key: 'control', label: 'Control', w: 0.24 },
      { key: 'estado', label: 'Estado', w: 0.15 },
      { key: 'evidencia', label: 'Evidencia', w: 0.15 },
      { key: 'lectura', label: 'Lectura IA', w: 0.20 },
    ],
    buildAiPremiumControlRows(payload),
    { rowHeight: 43 }
  );

  doc.addPage();
  addAiPremiumHeader(doc, { title, tenantName, dateText, pageNumber: 4, totalPages });
  addAiPremiumSectionTitle(doc, 'Brechas y hallazgos', 'Brechas, hallazgos sugeridos y señales auditoras', 'Elementos que requieren validación humana.');
  ...
  addAiPremiumTable(
    ...
    [
      { key: 'id', label: 'ID', w: 0.08 },
      { key: 'severidad', label: 'Severidad', w: 0.12 },
      { key: 'brecha', label: 'Brecha', w: 0.30 },
      { key: 'evidencia', label: 'Evidencia/causa', w: 0.25 },
      { key: 'accion', label: 'Acción sugerida', w: 0.25 },
    ],
    buildAiPremiumGapRows(payload),
    { rowHeight: 44 }
  );

  addAiPremiumTable(
    doc,
    'Hallazgos sugeridos por IA',
    [
      { key: 'tipo', label: 'Tipo', w: 0.16 },
      { key: 'severidad', label: 'Severidad', w: 0.13 },
      { key: 'requisito', label: 'Requisito', w: 0.20 },
      { key: 'descripcion', label: 'Descripción', w: 0.28 },
      { key: 'recomendacion', label: 'Recomendación', w: 0.23 },
    ],
    payload.profile.auditorFindings.slice(0, 4).map((description, index) => ({
      tipo: index === 0 ? 'Observación' : 'Oportunidad de mejora',
      severidad: index === 0 ? 'alta' : 'media',
      requisito: payload.profile.criteria[index] || 'Requisito aplicable',
      descripcion: description,
      recomendacion: payload.profile.auditorActions[index] || 'Validar con auditor humano antes de registrar.',
    })),
    { rowHeight: 43 }
  );

  doc.addPage();
  addAiPremiumHeader(doc, { title, tenantName, dateText, pageNumber: 5, totalPages });
  addAiPremiumSectionTitle(doc, 'Conclusión', 'Conclusión auditora y plan de seguimiento', 'Recomendaciones de cierre, revisión humana y trazabilidad técnica.');

  doc.roundedRect(36, doc.y, doc.page.width - 72, 76, 14)
    .fill('#FFFFFF')
    .stroke('#DCE7F3');
  ...
  doc.fillColor('#4338CA')
    .fontSize(8)
    .font('Helvetica-Bold')
    .text('CONCLUSIÓN IA AUDITOR', 52, doc.y + 12);

  doc.fillColor('#0F172A')
    .fontSize(15)
    .font('Helvetica-Bold')
    .text(payload.readiness.label, 52, doc.y + 28, { width: 300 });

  doc.fillColor('#475569')
    .fontSize(9)
    .font('Helvetica')
    .text(payload.readiness.conclusion, 52, doc.y + 48, { width: doc.page.width - 104 });

  doc.y += 92;

  addAiPremiumTable(
    doc,
    'Plan sugerido para revisión humana',
    [
      { key: 'prioridad', label: 'Prioridad', w: 0.13 },
      { key: 'accion', label: 'Acción sugerida', w: 0.34 },
      { key: 'responsable', label: 'Responsable', w: 0.17 },
      { key: 'plazo', label: 'Plazo', w: 0.11 },
      { key: 'evidencia', label: 'Evidencia esperada', w: 0.25 },
    ],
    payload.profile.auditorActions.slice(0, 6).map((action, index) => ({
      prioridad: index < 2 ? 'alta' : 'media',
      accion: action,
      responsable: payload.profile.code === 'ISO27001' ? 'Responsable SGSI' : payload.profile.code === 'ISO9001' ? 'Responsable SGC' : 'Responsable ISO',
      plazo: index < 2 ? '15 días' : '30 días',
      evidencia: 'Registro actualizado, evidencia objetiva y aprobación del responsable.',
    })),
    { rowHeight: 42 }
  );
}
...
function streamAiAuditorPdfReport(res, result, tenant, locale) {
  const source = result || {};
  const tenantData = tenant || source.tenant || {};
  const localeValue = locale || source.locale || 'es';
  const en = String(localeValue || '').toLowerCase().startsWith('en');
  ...
  const standardForFile =
    source?.standard_code ||
    source?.iso ||
    source?.full_result_json?.standard_code ||
    source?.summary_json?.standard_code ||
    'iso';

  const filename = `${sanitizeAiAuditorPdfFileName(
    `${tenantName}-${standardForFile}-ia-auditor`
  )}.pdf`;
