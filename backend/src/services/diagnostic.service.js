...
async function getSchemaCapabilities() {
  const [
    ...
    actionHasPriority,
    actionHasOwner,
    semanticHasTargetLabel,
    semanticHasTargetCode,
    semanticHasScore,
    semanticHasReason,
    semanticHasRationale,
    semanticHasSnippet,
  ] = await Promise.all([
    ...
    columnExists('tenant_evidence_applicability_suggestions', 'target_label'),
    columnExists('tenant_evidence_applicability_suggestions', 'target_code'),
    columnExists('tenant_evidence_applicability_suggestions', 'score'),
    columnExists('tenant_evidence_applicability_suggestions', 'reason'),
    columnExists('tenant_evidence_applicability_suggestions', 'rationale'),
    columnExists('tenant_evidence_applicability_suggestions', 'snippet'),
  ]);
...
async function loadSemanticSuggestions(tenantId, controls, caps) {
  ...
  const result = await pool.query(
    ...
    SELECT
      ...
      s.source_type,
      s.source_id,
      ${caps.semanticHasTargetLabel ? 's.target_label' : 'NULL::text AS target_label'},
      ${caps.semanticHasTargetCode ? 's.target_code' : 'NULL::text AS target_code'},
      ${caps.semanticHasScore ? 's.score' : 'NULL::numeric AS score'},
      s.confidence,
      ${caps.semanticHasReason ? 's.reason' : caps.semanticHasRationale ? 's.rationale AS reason' : 'NULL::text AS reason'},
      ${caps.semanticHasSnippet ? 's.snippet' : 'NULL::text AS snippet'},
      s.status,
      s.created_at,
      d.file_name AS document_file_name,
      d.relative_path AS document_relative_path,
  ...
  for (const row of result.rows) {
    pushGrouped(map, row.tenant_control_id, {
      ...
      source_type: row.source_type,
      source_id: row.source_id,
      name: row.document_file_name || row.document_relative_path || row.evidence_file_name || row.evidence_description || row.target_label || row.target_code || 'Evidencia sugerida',
      score: row.score,
