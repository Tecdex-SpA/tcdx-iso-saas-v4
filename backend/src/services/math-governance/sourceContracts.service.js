...
const SOURCE_CONTRACTS = Object.freeze([
  ...
  contract({
    ...
    columns: ['tenant_id','severity','status','created_at','opened_at','closed_at','completed_at','due_date','due_at','progress_percent','latest_progress_percent','latest_status_after','latest_update_at','period_start','period_end','generated_at','approved_evidence_count','pending_evidence_count','weight'], required_fields: ['id','tenant_id','status'],
    ...
    availability: 'available', version: 9,
    limitations: 'Acciones y remediación usan action_plans enriquecidos con el último action_plan_updates válido; hallazgos snapshot de readiness no tienen status operacional y se marcan not_applicable; su temporalidad proviene de grc_readiness_snapshots.period_start/period_end/generated_at, sin source_as_of físico; progreso ausente queda unmeasured y no se convierte en cero.'
  }),
  contract({
