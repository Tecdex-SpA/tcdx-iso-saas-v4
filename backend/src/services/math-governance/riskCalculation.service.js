...
function lossSeverity({ netLosses = [] }) { return { value: mean(netLosses), median: percentile(netLosses, 0.5), p90: percentile(netLosses, 0.9), p95: percentile(netLosses, 0.95), p99: percentile(netLosses, 0.99) }; }
function parametricVaR({ mean: mu, z, sigma }) { return number(mu, 'mean') + number(z, 'z') * number(sigma, 'sigma'); }
function fmeaRpn({ severity, occurrence, detection }) { return number(severity, 'severity') * number(occurrence, 'occurrence') * number(detection, 'detection'); }
function normalizeScale(value, { min = 1, max = 5, inverse = false } = {}) { const normalized = (number(value, 'value') - min) / (max - min); return clamp(inverse ? 1 - normalized : normalized, 0, 1); }
function classifyRisk(value, thresholds = { low: 4, medium: 9, high: 15 }) { const score = number(value, 'riskValue'); if (score >= thresholds.high) return 'critical'; if (score >= thresholds.medium) return 'high'; if (score >= thresholds.low) return 'medium'; return 'low'; }
function multidimensionalImpact(dimensions = {}, weights = {}) { const keys = Object.keys(dimensions); if (!keys.length) throw new MathGovernanceError('RISK_IMPACT_REQUIRED', 'Impacto multidimensional requiere dimensiones.'); const normalizedWeights = keys.reduce((acc, key) => ({ ...acc, [key]: weights[key] ?? 1 / keys.length }), {}); const total = Object.values(normalizedWeights).reduce((sum, value) => sum + number(value, 'weight'), 0); if (Math.abs(total - 1) > 0.0001) throw new MathGovernanceError('RISK_WEIGHTS_SUM_INVALID', 'Pesos de impacto deben sumar 1.'); return keys.reduce((sum, key) => sum + number(dimensions[key], key) * number(normalizedWeights[key], `weights.${key}`), 0); }
function officialInherentRisk(input = {}) { return officialResult('F5_5_INHERENT_RISK', input, { period: input.period || {}, source: input.source || null, components: { method: input.method || 'matrix', scale: input.scale || '5x5' }, explanation: 'Riesgo inherente oficial como probabilidad por impacto normalizado.' }); }
function officialResidualRisk(input = {}) { return officialResult('F5_5_RESIDUAL_RISK', input, { period: input.period || {}, source: input.source || null, components: { method: input.method || 'matrix', controlEffectiveness: input.controlEffectiveness }, explanation: 'Riesgo residual oficial ajustado por efectividad de control.' }); }
