...
async function main() {
  ...
  assert.throws(() => executeFormula('F5_5_RESIDUAL_RISK', residualMissingControl), /required|null/i, 'missing control effectiveness must not become zero');
  ...
  assert.ok(missingTables.reason.includes('No existen'));
