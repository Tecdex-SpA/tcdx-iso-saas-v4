...
function addIssue(collection, rowIndex, field, code, message, value = null) {
  collection.push({ row_index: rowIndex, field, code, message, value });
}
function validateTimezone(timezone) {
  ...
  try {
    ...
    throw new MathGovernanceError('DATASET_TIMEZONE_INVALID', 'Timezone invalido para dataset operacional.', { timezone });
  }
}
function validateDataset({
  ...
  if (periodStart && periodEnd && periodEnd < periodStart) {
    throw new MathGovernanceError('DATASET_PERIOD_INVALID', 'Periodo de dataset invalido.', { sourceKey });
  }
  if (expectedUnit && unit && expectedUnit !== unit) {
    warnings.push({ code: 'unit_mismatch', expected: expectedUnit, received: unit });
  }
  if (expectedCurrency && currency && expectedCurrency !== currency) {
    warnings.push({ code: 'currency_mismatch', expected: expectedCurrency, received: currency });
  }
  rows.forEach((row, index) => {
    ...
    for (const field of requiredFields) {
      if (row[field] === undefined || row[field] === null || row[field] === '') addIssue(issues, index, field, 'required_missing', 'Campo obligatorio ausente.', row[field]);
    }
    if (naturalKey) {
      const key = Array.isArray(naturalKey) ? naturalKey.map((field) => row[field] ?? '').join('|') : row[naturalKey];
      if (key) {
        if (seen.has(String(key))) addIssue(issues, index, Array.isArray(naturalKey) ? naturalKey.join(',') : naturalKey, 'duplicate_natural_key', 'Clave natural duplicada.', key);
        seen.add(String(key));
      }
    }
    ...
    for (const [field, rule] of Object.entries(scaleRules || {})) {
      if (row[field] === undefined || row[field] === null || row[field] === '') continue;
      const value = Number(row[field]);
      if (Number.isFinite(value) && rule.decimals !== undefined) {
        const decimals = String(row[field]).split('.')[1]?.length || 0;
        if (decimals > rule.decimals) addIssue(issues, index, field, 'scale_exceeded', 'Escala decimal excedida.', row[field]);
      }
    }
    ...
    for (const field of ['created_at', 'updated_at', 'measured_at', 'assessed_at', 'event_date', 'submitted_at', 'executed_at', 'tested_at']) {
    ...
    if (issues.length) {
      invalidRows.push({ row_index: index, issues });
      exclusions.push(...issues);
    }
  });
  ...
  return {
    ...
    unit,
    tenantId,
    status: sampleSize === 0 ? 'empty_dataset' : (valid ? 'ready' : 'validated_with_warnings'),
  };
}
