...
| -- | ------ | -------- | --------- | ------ | ----------: | -------------- | ------------- |
| B8-01 | IA frontend | `/ia` conserva recomendaciones legacy por control y contrato desalineado con su UI; IA.3 reemplazo `PUT /api/ai/apply/:tenant_control_id` por borrador revisable de plan de accion, sin archivar `/ia`. | Alta | `blocked_pending_ia4_read_migration_or_archive` | No | IA.4 debe migrar la lectura util de accion/evidencia/prioridad/explicacion a `/ia-compliance/sugerencias` o documentar paridad antes de archivar `/ia`. | Fase IA |
...
| B8-03 | Dashboard legacy | `/dashboard-v2` retirado antes de Phase 7 junto con validadores y API V2. | Baja | `resolved_pre_phase7` | No | Mantener guard `test:prephase7-cleanup`. | Dashboard QA |
