...
- RBAC-01 + BRAND-01: aprobado y versionado en `main@31ce8ffa95eede2d9986692dcc1c7eed18acac88` (`feat(auth): normalize RBAC commercial gating and product branding`). DB audit real consumido desde `artifacts/rbac01-db-audit/*.csv`; no se reconectó a PostgreSQL ni se modificó producción. Se implementó resolver backend/frontend de roles canónicos/compatibilidad preservando `effective_role` legacy; revisión final corrigió el match de role gates para que legacy/deprecated no satisfagan listas canonical-only por familia. `auditor` recupera dashboard; `admin != tenant_admin`, `superadmin != platform_admin` y `operativo` legacy quedan documentados/testeados. Comercial separa permiso, entitlement, módulo activo y scope; `MODULE_NOT_ACTIVE` deniega capabilities aunque estén entituladas. BRAND-01 normaliza superficies visibles actuales a `Tecdex GRC Compliance`; identificadores técnicos `tcdx/TECDX` preservados. Migración aditiva creada pero no ejecutada: `database/migrations/20260827_rbac01_canonical_roles_brand01.sql`. Runner operacional registrado localmente: `scripts/deploy-vms.sh -> scripts/rbac01/apply-rbac01-migration.js -> database/migrations/20260827_rbac01_canonical_roles_brand01.sql`; deploy/migración productiva no ejecutados por Codex. Siguiente gate: revisión humana, commit/push del runner, deploy oficial y validación runtime RBAC.
- Fase 7: NOT_STARTED.
...
5. RBAC-01 + BRAND-01 está en `main@31ce8ffa95eede2d9986692dcc1c7eed18acac88`; runner de migración RBAC-01 queda listo para revisión humana local. Usuario decide commit/push/PR/CI/deploy y aplicación/validación runtime de la migración.
