...
- RBAC-02 Commercial Gating: RBAC02_MIGRATION_RUNNER_READY_FOR_REVIEW en `main@ad608a9eaa89b1668281d7162d1bf3ff6aee1f6c` sin commit por instrucción. Root cause confirmado: `/dashboard` requiere `core.dashboard`/`core`, pero tenants comerciales históricos pueden no tener fila técnica de módulo `core`, causando falso `MODULE_NOT_ACTIVE`; runtime actual muestra `dashboard_module_mismatches=0`, por lo que la excepción queda como compatibilidad estricta sólo para `core.dashboard`. Auditoría READ-ONLY real consumida: 14 tenants, 50 users, 12 roles, 6 active subscriptions, 188 tenant capabilities, 0 unknown roles, 0 module mismatches, 0 false DENY y 1 false ALLOW risk pre-fix por `core.dashboard.required_permission=commercial.entitlement.read`. Se normalizó en resolver a `dashboards.read`, frontend respeta `rbac_allowed=false`, matriz rutas runtime/static queda sin divergencias y se creó migración forward-only no ejecutada `database/migrations/20260827_rbac02_commercial_gating_normalization.sql`. Cierre operacional agregado: runner `scripts/rbac02/apply-rbac02-migration.js` con migration id `20260827_rbac02_commercial_gating_normalization`, checksum, advisory lock, ledger `schema_migrations`, `--preflight`/`--apply`, postcondition `core.dashboard -> dashboards.read`, y registro oficial en `scripts/deploy-vms.sh` después de RBAC-01 y antes del deploy de servicios. Validaciones locales PASS: tests backend RBAC-02/RBAC-01/commercial/middleware, frontend lint/typecheck/sidebar-rbac/commercial-multitenant/build, runner syntax/checksum, deploy bash syntax, registro grep y `git diff --check`. Producción no fue modificada por Codex y la migración productiva sigue NOT_EXECUTED. Handoff: `docs/codex/handoffs/RBAC-02-COMMERCIAL-GATING.md`.
- Fase 7: NOT_STARTED.
...
6. RBAC-02 queda listo para revisión humana con runner de migración registrado; el usuario decide Git closeout, CI, deploy oficial, aplicación de `20260827_rbac02_commercial_gating_normalization` vía runner oficial y validación runtime.
