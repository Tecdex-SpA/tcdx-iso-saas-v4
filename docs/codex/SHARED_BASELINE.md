...
- 6.10-05 cierra localmente RAG grounded answers/citations: `rag-grounded-answer-contract-v1` en `backend/src/services/knowledge-base/knowledgeRag.service.js` reutiliza Hybrid Retrieval como única fuente de candidatos, materializa texto sólo para candidatos tenant-scoped seleccionados, construye contexto evidence-only, invoca AI Engine por `AiEngineClient`/`call_llm_json`, valida citations determinísticamente y abstiene por evidencia insuficiente sin crear persistencia ni truth paralela.
...
- Runtime validation de RAG grounded answer/citations 6.10-05.
