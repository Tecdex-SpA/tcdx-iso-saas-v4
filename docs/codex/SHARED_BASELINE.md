...
- 6.8-01-HF2 corrige el bootstrap runtime faltante del contrato semántico global `grc.manual_observations`: HF1 architectural reconciliation permanece válida, pero instalaciones ya desplegadas requerían una forward migration registrada para crear/reutilizar `grc.manual_observations@v1` y asignar `current_version_id`. `6.8-02` queda bloqueado hasta evidencia PostgreSQL post-deploy.
...
- No redescubrir en 6.8-02 el bootstrap de observaciones manuales tras HF2: `grc.manual_observations` debe existir como contrato global `tenant_id=NULL`, v1 `published`, con `current_version_id` apuntando a v1 antes de habilitar productores/outbox.
