...
- 6.10-04 cierra localmente Hybrid Retrieval: `hybrid-retrieval-contract-v1` en `backend/src/services/knowledge-base/knowledgeHybridRetrieval.service.js` combina lexical tenant-scoped sobre `knowledge_document_chunks` y vector tenant-scoped sobre `knowledge_chunk_embeddings`, aplica ranking determinístico versionado por normalización de rank y retorna candidatos/provenance sin RAG answer/citations.
...
- Hybrid Retrieval v1 existe localmente: usar `knowledgeHybridRetrieval.service.js` como orquestador de candidatos; no crear otro índice híbrido ni motor paralelo.
