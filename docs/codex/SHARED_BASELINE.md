...
- 6.10-01 cierra localmente el modelo canónico de documentos de conocimiento: `knowledge_documents` extiende Knowledge Base v2 como parent/source documental versionado y tenant-aware; `knowledge_sources.knowledge_document_id` enlaza fuentes KB v2 sin reemplazar `knowledge_items` ni child tables. El service boundary está en `backend/src/services/knowledge-base/knowledgeDocument.service.js`; no hay segunda KB, no hay auto-promoción de attachments operacionales, no hay pgvector/embeddings/retrieval/RAG en 6.10-01. Runtime queda pendiente de deploy/migración del usuario.
