...
- 6.10-03 cierra localmente la foundation pgvector/embeddings: `knowledge-embedding-contract-v1` en `backend/src/services/knowledge-base/knowledgeEmbedding.service.js` persiste embeddings en `knowledge_chunk_embeddings`, referencia `knowledge_document_chunks` sin copiar `chunk_text`, requiere provider/model/version/dimensions explícitos, conserva estados `ready/failed/stale/skipped` y filtra `tenant_id` antes del ranking vectorial. No implementa Hybrid Retrieval/RAG/citations/reranker.
