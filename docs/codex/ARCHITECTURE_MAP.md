...
Backend Node/Express
        ...
        |      + AI Core runtime context grants local: las vistas canonicas `ai_core.v_tenant_health_context`, `v_control_context`, `v_finding_context` y `v_kpi_context` se exponen al backend mediante grants explicitos `SELECT` al rol NOLOGIN `tcdx_backend_runtime`; `tcdx_backend_app` hereda por membresia existente y `ai_reader` no recibe acceso amplio a `ai_core`
        |
