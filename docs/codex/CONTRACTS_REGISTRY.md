...
| Capability/RBAC | CURRENT/RBAC-03-NOT-REQUIRED | A+C | Backend autoriza; roles canónicos usan resolver de compatibilidad sin alias de privilegios; autorización efectiva separa usuario, rol efectivo, permiso, tenant, suscripción/plan, módulo, capability y scope. RBAC-02 mantiene excepción estricta sólo para `core.dashboard` como base capability de tenant comercial activo con `dashboards.read`. RBAC-03 no requiere reconciliación de roles/permisos para el incidente confirmado; el fix corresponde a sincronización de contrato comercial y cache/context validation. |
