...
| AI Core runtime context view grants | CURRENT/AI-CORE-RUNTIME-GRANTS-LOCAL | CODEX A+B | `tcdx_backend_runtime` is the NOLOGIN runtime role that receives explicit allowlist `SELECT` grants on `ai_core.v_tenant_health_context`, `ai_core.v_control_context`, `ai_core.v_finding_context` and `ai_core.v_kpi_context`; `tcdx_backend_app` gets access only by existing role inheritance. No direct grant to `tcdx_backend_app`, no grant to `ai_reader`, no DML, no `GRANT ALL`, no schema-wide `ON ALL TABLES`, no ownership change, and no legacy view/column compatibility. |
