...
|---|---|---|---|
| LLM traceability actor/system/company | CURRENT/READY_FOR_REVIEW | CODEX A+B | Backend builds a canonical trace context before AI Engine calls: `actor` is resolved from server-side authenticated user email or stable process actor, `system='tcdx-iso'`, and `company` comes from canonical tenant/profile data. Spoofed frontend email/company/system/llm_trace is overwritten. AI Engine forwards actor through OpenAI `user` and `X-OpenWebUI-User-Email`, and for OpenAI-compatible calls sends metadata/tags for system/company/tenant/user when supported. No gateway URL/API key/model alias changes. Local regressions cover human/process actor, anti-spoofing, canonical company, multi-tenant company isolation, and semantic evidence trace propagation; runtime trace UI validation remains postdeploy. |
| Semantic evidence analysis / associations | CURRENT/READY_FOR_REVIEW | CODEX A+B+C | Existing AI Engine `POST /semantic-evidence/analyze` returns deterministic base plus optional `call_llm_json` enrichment through central `llm_client`, using only document text and backend-provided tenant-scoped candidate targets. Output is assistive and always requires human review: summary, document facts, AI inferences, classification, chunks, suggestions, scoring, usefulness, limitations and trace. Backend persists semantic profile/chunks/suggestions to canonical tenant evidence tables, discards suggestions whose target is not in the authorized candidate set, and creates `tenant_document_object_links` only after explicit human accept/review for `control`, `nonconformity` or `finding`. Associations do not change compliance, Health, control status, NC status or finding status. |
| Canonical document index / dashboard backend dataflow | CURRENT/READY_FOR_REVIEW | CODEX A+C | Manual evidence-library uploads persist in canonical `document_index` only: `provider='manual_upload'`, `source_id=NULL`, `integration_id=NULL`, tenant-scoped `local_storage_path`, coherent `checksum/content_hash/file_hash`, and `metadata_json.manual_upload=true`; `tenant_document_sources` is not required and is not created. Manual source cards are virtual projections from `document_index`. `PUT /api/controls/:id` updates only tenant-scoped `tenant_controls`, records SoA assessment, commits, then publishes official post-mutation propagation; no legacy `UPDATE controls` fallback or compatibility metadata remains in that endpoint. Dashboard control existence is `tenant_controls + tenant_applicable_controls`; `v_iso_control_effective_health` is optional enrichment and cannot hide controls or fabricate Health from status. |
| ISO9001 data/evidence systemic review | BLOCKED/LOCAL-PARTIAL | CODEX A+C | Repository authority remains `database/reference/iso/manifest.json`: `ISO9001:2015` is approved/certifiable/product-projected with 16 controls and 13 evidence expectations; `ISO9001:2026_FDIS` is transition-only with 8 controls and 8 evidence expectations, `product_standard_code=NULL`, and no projection into `standards`/`controls_catalog`. Codex did not invent the requested ~50-control catalog. Local repairs: dashboard controls use applicable tenant controls as universe with optional Health, legacy control update endpoint writes canonical `tenant_controls` first and publishes orchestration, evidence upload unlinks disk files on failed validation/transaction. |
...
| AI Core runtime context view grants | CURRENT/AI-CORE-RUNTIME-GRANTS-ALLOWLIST-LOCAL | CODEX A+B | `tcdx_backend_runtime` is the NOLOGIN runtime role that receives explicit allowlist `SELECT` grants on `ai_core.v_tenant_health_context`, `ai_core.v_control_context`, `ai_core.v_finding_context` and `ai_core.v_kpi_context`; `tcdx_backend_app` gets access only by existing role inheritance. The runner's unexpected privilege gate allows only the prior External Lookup runtime contract (`external_lookup_extra_charges`, `external_lookup_logs`, `external_lookup_quota_audit`, `external_lookup_quotas`, `v_external_lookup_usage_monthly`) plus those four context views. Any other `ai_core` SELECT remains fail-closed. No direct grant to `tcdx_backend_app`, no grant to `ai_reader`, no DML, no `GRANT ALL`, no schema-wide `ON ALL TABLES`, no ownership change, and no legacy view/column compatibility. |
| AI Engine canonical context builder | CURRENT/AI-CONTEXT-CANONICAL-SCHEMA-LOCAL | CODEX B | `ai-engine/app/services/context_builder.py` consumes current canonical `ai_core` context views without legacy DB compatibility: `v_tenant_health_context` and `v_kpi_context` are metric-snapshot based (`tenant_id`, `metric_code`, `numeric_value`, `publication_state`, `coverage`, `effective_at`); `v_control_context` exposes tenant/control/catalog identity, `code`, `title`, `standard_code`, `implementation_status`; `v_finding_context` exposes tenant/finding/control identity, title, severity, status and created_at. Control context buckets are derived in application from `implementation_status`. Missing evidence/finding/action counts are not treated as zero, and no cross-tenant fallback is allowed. |
| AI Guided canonical knowledge | CURRENT/AI-GUIDED-CANONICAL-KNOWLEDGE-LOCAL | CODEX B | `ai-engine/app/services/canonical_knowledge_service.py` is the Guided AI semantic adapter over KB v2/reference knowledge: `knowledge_items`, `knowledge_mappings`, `knowledge_common_gaps`, `knowledge_recommended_actions`, `knowledge_evidence_expectations`, `knowledge_rules`, `knowledge_rule_hints`, `knowledge_audit_questions`, `knowledge_sources`, `iso_evidence_expectations`, `tenant_applicable_evidence_requirements` and optional `recommendation_decision_ledger`. It replaces removed runtime reads to legacy expert objects without compatibility schema. Tenant-specific evidence and feedback require `tenant_id`; no cross-tenant fallback is allowed. Missing knowledge/feedback/scenario remains insufficiency/provenance and `can_auto_close=false`. Audit questions/rules are human validation aids only. Ledger decisions do not imply usefulness/effectiveness, and external lookup trusted sources require explicit trust metadata plus `allowed_domains`. |
| AI Guided canonical knowledge runtime grants | CURRENT/AI-GUIDED-CANONICAL-KNOWLEDGE-RUNTIME-GRANTS-LOCAL | CODEX A+B | Forward-only migration `20260914_ai_guided_canonical_knowledge_runtime_grants` grants exactly `SELECT` on `public.knowledge_audit_questions`, `public.knowledge_common_gaps`, `public.knowledge_evidence_expectations`, `public.knowledge_mappings`, `public.knowledge_recommended_actions`, `public.knowledge_rule_hints` and `public.knowledge_rules` to NOLOGIN `tcdx_backend_runtime`. Preexisting legitimate runtime reads remain `public.iso_evidence_expectations`, `public.knowledge_items`, `public.knowledge_sources`, `public.recommendation_decision_ledger` and `public.tenant_applicable_evidence_requirements`. `tcdx_backend_app` must inherit through role membership only; no direct app grant, no `ai_reader`, no DML, no schema-wide grants and no ownership changes. Runner pre/apply is convergent for already-present target SELECT grants but fails closed on unsafe privileges, checksum mismatch, broken role contract or unexpected public SELECT outside the versioned fresh-deploy allowlist. |
...
| AI Engine DGX/OpenAI-compatible provider | CURRENT/DGX-INTEGRATION-HARDENING-LOCAL | CODEX B | `openai_compatible` reutiliza `OPENAI_BASE_URL`, `OPENAI_API_KEY` y `OPENAI_MODEL`; envia JSON mode, `reasoning_effort` configurable default `low`, `max_tokens` configurable con minimo efectivo 2000, parsea solo `message.content`, ignora `reasoning_content` como salida funcional y no propaga opciones Ollama. Narrative sparse-context grounding devuelve insuficiencia explicita/human review; con evidencia operacional, grounding se valida mediante `grounding_claims`/`claims` con `source_refs` autorizadas por el contexto, sin blacklist de palabras del ejemplo DGX. |
...
| TCDX SaaSv2 runtime contract V2 | CURRENT/TCDX-SAASV2-RUNTIME-CONTRACT-V2-LOCAL | CODEX A | `tenant_standards` agrega `catalog_mode` y lifecycle timestamps sin retirar `activated_at`; `tenant_nonconformities.control_id` es catalog identity y `tenant_control_id` operational identity; `evidences.control_id` y `catalog_control_id` son aliases catalog reconciliados. Triggers permiten inserts activos sin `title`, rellenan compatibilidad desde descripcion/nombre archivo y bloquean divergencia catalog/tenant-control. |
| Evidence AI runtime DB contract | CURRENT/TCDX-SAASV2-RUNTIME-CONTRACT-V2-LOCAL | CODEX A | Runtime usa `evidence_document_extracts`, `vw_evidence_current_extracts`, `evidence_ai_assessments`, `vw_evidence_current_ai_assessments`, `evidence_ai_jobs`, `enqueue_evidence_ai_job(uuid,uuid,text,jsonb,smallint,timestamp,uuid)` y `evidence_knowledge_chunks`, con FKs tenant-aware y grants explicitos a `tcdx_backend_runtime`. |
| Standard lifecycle runtime DB contract | CURRENT/TCDX-SAASV2-RUNTIME-CONTRACT-V2-LOCAL | CODEX A | Stage catalog canonico: `diagnostico`, `diseno_planificacion`, `implementacion`, `verificacion_auditoria`, `certificacion`, `mejora_continua`, `suspendida_fuera_alcance`. Runtime usa `standard_lifecycle_status`, `standard_lifecycle_snapshots`, `standard_lifecycle_stage_requests` y `standard_lifecycle_ai_feed` con clave logica `(tenant_id, standard_code, operation_id)` y FKs tenant-aware. |
| TCDX SaaSv2 GRC runtime contract V3 | CURRENT/TCDX-SAASV2-GRC-RUNTIME-CONTRACT-V3-LOCAL | CODEX A | `saas_modules` incorpora `default_enabled`, `is_system`, `updated_at`; `tenant_module_settings.is_enabled` es autoridad runtime y `enabled` queda como alias sincronizado por trigger. Baseline/V3 incorporan contratos GRC Phase 1/2/3 consumidos por backend activo, discovery SQL real de schedulers Phase 1/2, grants explicitos a `tcdx_backend_runtime`, FKs tenant-aware criticas de conectores y seeds globales deny-by-default sin tenants. |
| TCDX SaaSv2 fresh deploy contract | CURRENT/TCDX-SAASV2-FRESH-DEPLOY-ARCHITECTURE | CODEX A | `scripts/deploy-vms.sh` separa `fresh-baseline` de `historical-upgrade`. `fresh-baseline` requiere `current_database=tcdx_saasv2`, rechaza la cadena historica, no ejecuta baseline/seed/reference loader en deploy recurrente y permite cero migraciones forward-only pendientes. `historical-upgrade` preserva runners historicos para bases no fresh y aborta si apunta a `tcdx_saasv2`. El deploy valida tambien backend runtime DB desde `.env` sin imprimir secretos antes y despues de reiniciar backend. |
| TCDX SaaSv2 fresh runtime contract closeout | CURRENT/TCDX-SAASV2-FRESH-RUNTIME-CONTRACT-CLOSEOUT-LOCAL | CODEX A | Migración forward-only `20260910_tcdx_saasv2_fresh_runtime_contract_closeout` con checksum `09201f0233cbad14cdc071f150b52ef80b6a1f847fef02b75631fc621b847ca5` alinea fresh DB con runtime activo de perfil, Admin SaaS, RBAC SQL, dealers, módulos, contratos, superficies comerciales administrables, prefacturación, external lookup y auditoría. No usa `DROP VIEW ... CASCADE`, no seed de precios ni cuota default, no hardcode tenant/user/email/UUID, no `GRANT ON ALL TABLES` ni `GRANT EXECUTE ON ALL FUNCTIONS`; runner soporta checksum/preflight/apply/idempotencia/mismatch fail-closed y está registrado sólo en `FRESH_PRODUCTION_MIGRATION_RUNNERS`. |
| TCDX commercial runtime integral closeout | CURRENT/TCDX-COMMERCIAL-RUNTIME-INTEGRAL-CLOSEOUT-LOCAL | CODEX A | Migración forward-only `20260910_tcdx_commercial_runtime_integral_closeout` con checksum `4310372bbd795eef116b14110283d16e0ef7a247ee81d51b54de7d4a6debe51d` cubre contratos físicos activos: `tenant_standard_audit`, `tenant_company_profiles`, `tenant_applicable_controls`, `tenant_document_object_links`, `findings.due_date` y `tenant_subscription_addons.tenant_id`. `normalize_status_for_audits(text)` queda eliminado como dependencia runtime mediante normalización SQL inline; `search_history` degrada seguro si falta la relación. Runner checksum/preflight/apply/idempotencia/mismatch fail-closed registrado sólo en `FRESH_PRODUCTION_MIGRATION_RUNNERS`; PostgreSQL aislado PASS. |
| Control lifecycle systemic closeout | CURRENT/TCDX-CONTROL-LIFECYCLE-SYSTEMIC-CLOSEOUT-LOCAL | CODEX A+C | Contrato canonico: catalogo global = `controls_catalog.tenant_id IS NULL`; catalogo tenant = `controls_catalog.tenant_id=current tenant`; `generic` selecciona global activo, `personalized` same-tenant activo y `mixed` ambos, deduplicando solo reemplazos declarados por `COALESCE(base_control_id,id)`. `source_type` es provenance, no autoridad funcional. Runtime usa `backend/src/services/controlCatalogLifecycle.service.js`; fresh/backfill usa `public.tcdx_effective_control_catalog(uuid,text,text)` en `20260911_tcdx_control_lifecycle_systemic_closeout` checksum `e3912a97fb9e57177b5e94e9946169c3bd055d2fb3fa194619644f7c64ba147b`. La pertenencia normativa normaliza case/whitespace en helper Node y funcion SQL. `initialize-controls` materializa `tenant_controls` y aplicabilidad idempotente bajo la norma solicitada sin scores, sin sobrescribir exclusiones humanas/perfil y alimenta consumidores por `tenant_controls -> tenant_applicable_controls -> v_iso_control_effective_health`. |
| Post-lifecycle runtime consumers closeout | CURRENT/TCDX-POST-LIFECYCLE-RUNTIME-CONSUMERS-LOCAL | CODEX A+C | Migracion forward-only `20260911_tcdx_post_lifecycle_runtime_consumers_systemic_closeout` checksum `1d9e00b12bc1d05b580b3002b07ff9db318e24ef578301d439d6cbaad85cdebe` materializa vistas Health tenant-scoped reales: `v_control_health_risks`, `v_control_health_risks_applicable`, `v_health_root_causes_by_standard`, `v_health_remediation_summary_by_tenant`, `v_health_remediation_summary_by_standard`, `v_health_remediation_plan`, `v_remediation_executive_by_tenant`, `v_remediation_executive_by_standard`, `v_evidence_approval_queue`, `v_controls_recovered_by_remediation`, `v_audit_event_log_enriched`, `v_audit_action_plan_timeline`, `v_audit_evidence_timeline` y `v_audit_control_recovery_timeline`. Runtime elimina dependencia de `controls_catalog_standards.clause`, `display_clause` y `refresh_kpi_health_snapshots(uuid)`, preserva no-data como NULL/sin_datos, y degrada capabilities deshabilitadas como estado contractual. Runner preflight pendiente no consulta postcondiciones de vistas inexistentes; preflight aplicado si verifica vistas/grants/NOLOGIN. IA Compliance mantiene `ai.view` + `ai.compliance` en endpoints protegidos y deja `/engine-health` como estado tecnico/contractual. |
| Release RBAC / capability systemic closeout | CURRENT/FINAL_PREDEPLOY_LOCAL_READY_FOR_HUMAN_REVIEW | CODEX A+C | Contrato de autorizacion efectiva: `usuario -> rol canonico -> permiso RBAC -> tenant scope -> entitlement/capability -> acceso efectivo`. Artefactos en `artifacts/release-rbac/`; migracion candidate `20260910_release_rbac_capability_systemic_closeout` checksum `8265da4cbf70aa9b2222456e7cc23f9ff5f38cb4268b4a42f7feea836243091b`. Static contract PASS con unsafe executable legacy platform checks 0, residual authorization authority 0, unclassified role checks 0, runtime permissions 121, matrix permissions 175, catalog permissions 203, runtime missing/unexpected 0, capability/role/permission orphans 0, contract permission unclassified 0. PostgreSQL aislado, siete roles tenant A/B, dealer assignment, entitlement enforcement, alias equivalence, frontend/backend consistency y backend suite PASS local. No commit/push/deploy/DB real. |
...
## Fresh baseline runtime dependency contracts — 2026-09-11

| Contract | Status | Authority | Validation |
|---|---|---|---|
| Fresh runtime dependency closeout | READY_FOR_HUMAN_REVIEW_LOCAL | `database/migrations/20260910_tcdx_fresh_baseline_runtime_dependency_systemic_closeout.sql` plus runner `scripts/normalization/apply-tcdx-fresh-baseline-runtime-dependency-systemic-closeout.js`; deploy order is fresh runtime contract -> Release RBAC -> Commercial runtime integral -> Fresh baseline runtime dependency. | Isolated PostgreSQL runner test PASS with baseline+seed+prior fresh migrations, first apply, reapply, checksum mismatch fail closed and checker mutation probes. |
| Controls initialization | CURRENT_LOCAL | `/api/admin-saas/tenants/:tenant_id/standards/:standard_code/initialize-controls` in `backend/src/routes/admin-saas.routes.js`; tenant standard is the canonical association, operation is assigned only when uniquely/default-resolved. | Duplicate route removed; no `tenant_controls.notes`; no fabricated numeric Health; idempotent by `(tenant_id, control_id)`. |
| Runtime dependency checker | CURRENT_LOCAL | `scripts/normalization/check-fresh-baseline-runtime-dependencies.js` combines bounded regex inventory, explicit relation/function/column/grant manifest, forbidden legacy checks and optional rollback mutation probes. | `--scan-only` PASS; isolated `--mutation-probes` PASS; scanner limitation is printed as `regex_inventory_not_full_sql_parser`. |
| Notifications and document index | CURRENT_LOCAL | `notifications`, `document_index`, `tenant_document_index_exclusions`; tenant-scoped rows, explicit indexes and runtime grants. Optional document source/integration same-tenant FKs are added when parent contracts exist. | Mutation probes insert/update notification, document index and active exclusion under `SET LOCAL ROLE tcdx_backend_runtime`. |
| Reports runtime catalog | CURRENT_LOCAL | `report_types`, `report_access_rules`, `report_exports`; report types/access are global product reference rows only. | Report access seed covers normalized roles used by `reports.routes.js`; no tenant/user/commercial data seeded. |
| ISO Express diagnostic | CURRENT_LOCAL | `iso_express_assessments`, items, gaps, answers and audit log, adapted from historical ISO Express contract with tenant FKs and child `(tenant_id, assessment_id)` constraints. | Mutation probes write assessment and children, and same-tenant FK cross-tenant violation fails closed. |
| ISO Operational Suggestions | CURRENT_LOCAL | `iso_operational_suggestions`, `iso_operational_suggestion_audit_log`, `iso_recommended_action_conversions`, queue/summary views. Runtime identity is `dedupe_key`; legacy `suggestion_key/source_type/recommendation` are nullable compatibility. | Mutation probes insert suggestion, audit log and conversion; queue view exposes payload/source_trace aliases plus resolved control context. |
| Per-control Health projection | CURRENT_LOCAL | `public.v_iso_control_effective_health` reads only `metric_snapshots.metric_code='F5_5_CONTROL_EFFECTIVENESS'` with explicit tenant control identity. | Runner and checker assert no `control_health_scores`, no KPI-HLT and presence of `category` from `controls_catalog.category`. |

## GRC runtime functional contracts — 2026-09-11

| Contract | Status | Authority | Validation |
|---|---|---|---|
| Action plan physical schema | CURRENT_LOCAL | Runtime writers use `backend/src/utils/actionPlanPersistence.js`; optional legacy fields are written only when real physical columns exist, otherwise preserved in `metadata`. Runtime projections recover description/priority/owner/approval status from physical row JSON or metadata. | Focal search found no active `action_plans.description`/`ap.description`/`a.description`; `node backend/src/utils/actionPlanPersistence.test.js` PASS. |
| Diagnostic semantic suggestions | CURRENT_LOCAL | `tenant_evidence_applicability_suggestions` optional columns are detected by schema capability; absent `target_label`, `score`, `reason` or `snippet` become nullable projections, not schema changes. | `node -c backend/src/services/diagnostic.service.js` PASS; `grcRuntimeRepair.contract.test.js` PASS. |
| Diagnostic SoA mutation | CURRENT_LOCAL | Diagnostic status update persists `tenant_controls.status` and records canonical SoA assessment with `tenant_controls.id`, active tenant standard code when available, and governed implementation/applicability fields. | `node -c backend/src/routes/diagnostic.routes.js` PASS; orchestration focal test PASS. |
| ISO Express standard eligibility | CURRENT_LOCAL | Active tenant standard identity is normalized from versioned/product codes to canonical `iso_standard_versions.standard_code` for options and calculation. | Contract test PASS; frontend selector consumes `/api/iso-express-diagnostic/options/:tenant_id`. |
| Post-mutation official publication | CURRENT_LOCAL | Mutations publish through `publishAffectedOfficialIndicators`, which calculates affected official metric codes and delegates to `indicatorGovernance`/official source contracts. Failure after commit returns observable `completed_with_failures`. | Isolated PostgreSQL formula lineage PASS: runs 18, outputs 18, source snapshots 18, metric snapshots 6, cross-tenant leakage 0, cross-standard leakage 0. |
| Per-control Health display | CURRENT_LOCAL | Control workbench consumes canonical/effective Health fields; missing Health stays `sin_datos` with null/N/A score. | Frontend typecheck PASS; contract test forbids score-threshold local derivation. |
| Action plan fallback strictness | CURRENT_LOCAL | `backend/src/utils/actionPlanPersistence.js` persists physical `action_plans` columns, preserves only explicitly governed optional absent fields in `metadata`, and throws `ACTION_PLAN_UNKNOWN_COLUMN` / `ACTION_PLAN_REQUIRED_COLUMN_MISSING` for unknown or structural drift. | `node backend/src/utils/actionPlanPersistence.test.js` PASS; focal grep shows no active `action_plans.description`/`ap.description`/`a.description` dependency. |
| ISO Express standard identity | CURRENT_LOCAL | `tenant_standards.standard_code` is matched to `iso_standard_versions` by generic normalization and version identity candidates; backend/frontend contain no norm-number whitelist for eligibility or display. | `node backend/src/routes/grcRuntimeRepair.contract.test.js` PASS; final ISO hardcode grep PASS. |
| Control workbench Health pair | CURRENT_LOCAL | Workbench API normalizes `effective_health_status` and `effective_health_score` as one canonical pair from `public.v_iso_control_effective_health`: null/non-finite score or `sin_datos` status returns `sin_datos` + `NULL`; measured status requires measured score. | Contract test PASS for null score, `sin_datos`, healthy filter parity, deteriorated/critical summary, null-excluding average and all-unmeasured average `NULL`. |
